...
/**
 * The main class for SSH client. It can only handle the following command line (which is used by Bazaar):
...
public class SSHMain implements BazaarExternalApp {
  /**
   * the semaphore
   */
   ...
   * the xml RPC port
   */
  private final BzrSSHXmlRpcClient myXmlRpcClient;
  /**
   * the command to run
   */
  ...
  private SSHMain(String host, String username, Integer port, String command) throws IOException {
    ...
    myHandlerNo = Integer.parseInt(System.getenv(BzrSSHHandler.SSH_HANDLER_ENV));
    int xmlRpcPort = Integer.parseInt(System.getenv(BzrSSHHandler.SSH_PORT_ENV));
    myXmlRpcClient = new BzrSSHXmlRpcClient(xmlRpcPort, myHost.isBatchMode());
    myCommand = command;
  }
  ...
  private class HostKeyVerifier implements ServerHostKeyVerifier {
    ...
    public boolean verifyServerHostKey(String hostname, int port, String serverHostKeyAlgorithm, byte[] serverHostKey) throws Exception {
      try {
        String s = System.getenv(BzrSSHHandler.SSH_IGNORE_KNOWN_HOSTS_ENV);
