...
import bazaar4idea.history.wholeTree.BzrBinaryMultipleContentsRevision;
import bazaar4idea.history.wholeTree.BzrMultipleContentsRevision;
...
public class BzrContentRevision implements ContentRevision {
  ...
  protected BzrContentRevision(@NotNull FilePath file,
                               @NotNull BzrRevisionNumber revision,
  ...
  public String getContent() throws VcsException {
    ...
    try {
      return ContentRevisionCache
        .getOrLoadAsString(myProject, myFile, myRevision, BzrVcs.getKey(), ContentRevisionCache.UniqueType.REPOSITORY_CONTENT,
  ...
  public boolean equals(Object obj) {
    ...
    BzrContentRevision test = (BzrContentRevision)obj;
  ...
  public static ContentRevision createMultipleParentsRevision(@NotNull Project project, @NotNull final FilePath file,
                                                              @NotNull BzrRevisionNumber currentRevision,
                                                              @NotNull final List<BzrRevisionNumber> parentRevisions) throws VcsException {
    final BzrContentRevision contentRevision = createRevisionImpl(file, currentRevision, project, null);
    if (parentRevisions.size() == 1) {
      return contentRevision;
    } else {
      if (contentRevision instanceof BzrBinaryContentRevision) {
        return new BzrBinaryMultipleContentsRevision(file, parentRevisions, (BzrBinaryContentRevision) contentRevision);
      } else {
        return new BzrMultipleContentsRevision(file, parentRevisions, contentRevision);
      }
    }
  }
  ...
  private static String makeAbsolutePath(@NotNull VirtualFile vcsRoot, @NotNull String path, boolean unescapePath) throws VcsException {
    final String unescapedPath = unescapePath ? BzrUtil.unescapePath(path) : path;
  ...
  private static BzrContentRevision createRevisionImpl(@NotNull FilePath path, @NotNull BzrRevisionNumber revisionNumber,
