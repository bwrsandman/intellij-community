 ...
 *         Date: 17.07.13
 */
public class BzrTaskHandler extends VcsTaskHandler {

  private final BzrBrancher myBrancher;
  private final BzrRepositoryManager myRepositoryManager;
  private final Project myProject;

  public BzrTaskHandler(BzrBrancher brancher, BzrRepositoryManager repositoryManager, Project project) {
    myBrancher = brancher;
  ...
  public TaskInfo startNewTask(final String taskName) {
    List<BzrRepository> repositories = myRepositoryManager.getRepositories();
    List<BzrRepository> problems = ContainerUtil.filter(repositories, new Condition<BzrRepository>() {
      @Override
      public boolean value(BzrRepository repository) {
  ...
  private static void fillMap(String taskName, List<BzrRepository> repositories, MultiMap<String, String> map) {
  ...
  public void switchToTask(TaskInfo taskInfo, Runnable invokeAfter) {
    for (final String branchName : taskInfo.branches.keySet()) {
      List<BzrRepository> repositories = getRepositories(taskInfo.branches.get(branchName));
      List<BzrRepository> notFound = ContainerUtil.filter(repositories, new Condition<BzrRepository>() {
        @Override
        public boolean value(BzrRepository repository) {
  ...
  private void merge(TaskInfo taskInfo) {
    for (String featureBranch : taskInfo.branches.keySet()) {
      myBrancher.merge(featureBranch, BzrBrancher.DeleteOnMergeOption.DELETE, getRepositories(taskInfo.branches.get(featureBranch)));
    }
  }
  ...
  public TaskInfo getActiveTask() {
    ...
    for (BzrRepository repository : repositories) {
      BzrLocalBranch branch = repository.getCurrentBranch();
      if (branch != null) {
  ...
  public TaskInfo[] getCurrentTasks() {
    List<BzrRepository> repositories = myRepositoryManager.getRepositories();
    final List<String> names = ContainerUtil.map(repositories, new Function<BzrRepository, String>() {
      @Override
      public String fun(BzrRepository repository) {
        return repository.getPresentableUrl();
      }
    });
    Collection<String> branches = BzrBranchUtil.getCommonBranches(repositories, true);
  ...
  private List<BzrRepository> getRepositories(Collection<String> urls) {
    final List<BzrRepository> repositories = myRepositoryManager.getRepositories();
    return ContainerUtil.mapNotNull(urls, new NullableFunction<String, BzrRepository>() {
      @Nullable
      @Override
      public BzrRepository fun(final String s) {

        return ContainerUtil.find(repositories, new Condition<BzrRepository>() {
          @Override
          public boolean value(BzrRepository repository) {
