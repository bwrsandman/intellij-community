...
/**
 * Basic abstract action handler for all Bazaar actions to extend.
 */
public abstract class BasicAction extends DumbAwareAction {
  ...
  public void actionPerformed(@NotNull AnActionEvent event) {
    ...
    if (!background) {
      BzrVcs.runInBackground(new Task.Backgroundable(project, getActionName()) {
        public void run(@NotNull ProgressIndicator indicator) {
          VcsFileUtil.refreshFiles(project, Arrays.asList(affectedFiles));
          UIUtil.invokeLaterIfNeeded(new Runnable() {
            public void run() {
              BzrUIUtil.showOperationErrors(project, exceptions, actionName);
            }
          });
        }
      });
  ...
  protected boolean toBackground(final Project project,
    ...
    BzrVcs.runInBackground(new Task.Backgroundable(project, getActionName()) {
      public void run(@NotNull ProgressIndicator indicator) {
        ...
        UIUtil.invokeLaterIfNeeded(new Runnable() {
          public void run() {
            BzrUIUtil.showOperationErrors(project, exceptions, getActionName());
          }
        });
      }
    });
  ...
  protected VirtualFile[] collectAffectedFiles(@NotNull Project project, @NotNull VirtualFile[] files) {
    ...
    for (VirtualFile file : files) {
      if (!file.isDirectory() && projectLevelVcsManager.getVcsFor(file) instanceof BzrVcs) {
   ...
   */
  protected abstract boolean isEnabled(@NotNull Project project, @NotNull BzrVcs vcs, @NotNull VirtualFile... vFiles);
