...
import bazaar4idea.history.BzrDiffFromHistoryHandler;
 ...
 */
public class BzrCompareWithBranchAction extends DumbAwareAction {

  private static final Logger LOG = Logger.getInstance(BzrCompareWithBranchAction.class.getName());
  ...
  public void actionPerformed(final AnActionEvent event) {
    ...
    if (currentBranch == null) {
      ...
      head = BzrUtil.getShortHash(currentRevision);
    }
    else {
  ...
  private static List<String> getBranchNamesExceptCurrent(BzrRepository repository) {
    List<BzrBranch> localBranches = new ArrayList<BzrBranch>(repository.getBranches().getLocalBranches());
    Collections.sort(localBranches);
    List<BzrBranch> remoteBranches = new ArrayList<BzrBranch>(repository.getBranches().getRemoteBranches());
    ...
    for (BzrBranch branch : localBranches) {
      branchNames.add(branch.getName());
    }
    for (BzrBranch branch : remoteBranches) {
      branchNames.add(branch.getName());
    }
    return branchNames;
  }
  ...
  public void update(AnActionEvent e) {
    ...
    BzrRepository repository = manager.getRepositoryForFile(vFiles[0]);
  ...
  private static boolean noBranchesToCompare(@NotNull BzrRepository repository) {
  ...
  private static class OnBranchChooseRunnable implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        if (e.getMessage().contains("exists on disk, but not in")) {
          ...
          BzrUIUtil.notifyError(myProject, "Couldn't compare with branch",
    ...
    private static void showDiffWithBranch(@NotNull Project project, @NotNull VirtualFile file, @NotNull String head,
      ...
      // we could use something like BzrRepository#getCurrentRevision here,
      // but this way we can easily identify if the file is available in the branch
      final BzrRevisionNumber currentRevisionNumber = (BzrRevisionNumber)BzrHistoryUtils.getCurrentRevision(project, filePath, head);
      final BzrRevisionNumber compareRevisionNumber =
        (BzrRevisionNumber)BzrHistoryUtils.getCurrentRevision(project, filePath, branchToCompare);
      ...
      final VcsFileRevision compareRevision =
        new BzrFileRevision(project, filePath, new BzrRevisionNumber(branchToCompare, compareRevisionNumber.getTimestamp()));
      CurrentRevision currentRevision = new CurrentRevision(file, new BzrRevisionNumber(head, currentRevisionNumber.getTimestamp()));
      new BzrDiffFromHistoryHandler(project).showDiffForTwo(new FilePathImpl(file), compareRevision, currentRevision);
    }
    ...
    private static void fileDoesntExistInBranchError(@NotNull Project project, @NotNull VirtualFile file, @NotNull String branchToCompare) {
      BzrUIUtil.notifyMessage(project, BzrUtil.fileOrFolder(file) + " doesn't exist in branch", String
        .format("%s <code>%s</code> doesn't exist in branch <code>%s</code>", BzrUtil.fileOrFolder(file), file.getPresentableUrl(),
                branchToCompare), NotificationType.WARNING, true, null);
    }
  }
