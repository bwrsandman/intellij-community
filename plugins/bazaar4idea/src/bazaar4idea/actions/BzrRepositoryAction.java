 ...
 */
public abstract class BzrRepositoryAction extends DumbAwareAction {
  /**
  ...
  public void actionPerformed(final AnActionEvent e) {
    ...
    final List<VirtualFile> roots = getBzrRoots(project, vcs);
    ...
    if (vFiles != null) {
      for (VirtualFile file : vFiles) {
        final VirtualFile root = BzrUtil.bzrRootOrNull(file);
        if (root != null) {
          defaultRootVar = root;
          break;
        }
  ...
  protected final void runFinalTasks(Project project, BzrVcs vcs, Set<VirtualFile> affectedRoots, String actionName,
   ...
   * false if the task is responsible to call the final tasks manually via {@link #runFinalTasks(Project, bazaar4idea.BzrVcs, Set, String, List)}.
   */
    ...
    if (project != null) {
      ...
      if (files != null) {
        for (VirtualFile file : files) {
          ...
          final BzrRepository repositoryForFile = manager.getRepositoryForFile(file);
          if (repositoryForFile != null && repositoryForFile.getState() == BzrRepository.State.REBASING) {
            return true;
          }
        }
      }
   ...
   */
  @Nullable
  public static List<VirtualFile> getBzrRoots(Project project, BzrVcs vcs) {
    List<VirtualFile> roots;
    try {
      roots = BzrUtil.getBzrRoots(project, vcs);
    }
    catch (VcsException e) {
      Messages.showErrorDialog(project, e.getMessage(),
                               BzrBundle.getString("repository.action.missing.roots.title"));
      return null;
    }
    return roots;
  }
