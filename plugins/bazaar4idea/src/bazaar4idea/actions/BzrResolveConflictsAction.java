...
/**
 * Bazaar merge tool for resolving conflicts. Use IDEA built-in 3-way merge tool.
 */
public class BzrResolveConflictsAction extends BzrAction {
  ...
  public void actionPerformed(@NotNull AnActionEvent event) {
    ...
    AbstractVcsHelper.getInstance(project).showMergeDialog(new ArrayList<VirtualFile>(conflictedFiles), BzrVcs.getInstance(project).getMergeProvider());
    for (BzrRepository repository : BzrUtil.getRepositoriesForFiles(project, conflictedFiles)) {
      repository.update();
    }
  }
