...
/**
 * Bazaar file annotation implementation
 * <p/>
 * Based on the JetBrains SVNAnnotationProvider.
 */
public class BzrFileAnnotation extends FileAnnotation {
  private final static Logger LOG = Logger.getInstance("#bazaar4idea.annotate.BzrFileAnnotation");
  ...
  private final LineAnnotationAspect DATE_ASPECT = new BzrAnnotationAspect(BzrAnnotationAspect.DATE, true) {
  ...
  private final LineAnnotationAspect REVISION_ASPECT = new BzrAnnotationAspect(BzrAnnotationAspect.REVISION, false) {
    @Override
  ...
  private final LineAnnotationAspect AUTHOR_ASPECT = new BzrAnnotationAspect(BzrAnnotationAspect.AUTHOR, true) {
    @Override
    protected String doGetValue(LineInfo lineInfo) {
   ...
   * @param revision
   */
  public BzrFileAnnotation(@NotNull final Project project,
  ...
  public String getToolTip(final int lineNumber) {
    ...
    if (fileRevision != null) {
      return BzrBundle
        .message("annotation.tool.tip", info.getRevision().asString(), info.getAuthor(), info.getDate(), fileRevision.getCommitMessage());
    }
    else {
      return "";
    }
  ...
  public void appendLineInfo(final Date date,
                             final BzrRevisionNumber revision,
                             final String author,
                             final String line,
   ...
   */
  private abstract class BzrAnnotationAspect extends LineAnnotationAspectAdapter {
    public BzrAnnotationAspect(String id, boolean showByDefault) {
    ...
    protected void showAffectedPaths(int lineNum) {
      if (lineNum >= 0 && lineNum < myLines.size()) {
        ...
        if (info != null) {
          ShowAllAffectedGenericAction.showSubmittedFiles(myProject, info.getRevision(), myFile, BzrVcs.getKey());
        }
      }
    }
  }
  ...
  static class LineInfo {
     ...
     * revision number
     */
    private final BzrRevisionNumber myRevision;
    /**
     ...
     */
    public LineInfo(final Date date, final BzrRevisionNumber revision, final String author) {
      myDate = date;
      myRevision = revision;
      myAuthor = author;
    }
     ...
     */
    public BzrRevisionNumber getRevision() {
      return myRevision;
    }
