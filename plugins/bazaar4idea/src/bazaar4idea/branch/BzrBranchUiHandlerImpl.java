 ...
 */
public class BzrBranchUiHandlerImpl implements BzrBranchUiHandler {
  ...
  public BzrBranchUiHandlerImpl(@NotNull Project project,
  ...
  public void notifySuccess(@NotNull String title, @NotNull String description, @Nullable NotificationListener listener) {
    Notificator.getInstance(myProject).notify(BzrVcs.NOTIFICATION_GROUP_ID, title, description, NotificationType.INFORMATION, listener);
  }

  @Override
  public void notifyError(@NotNull String title, @NotNull String message) {
    Notificator.getInstance(myProject).notify(BzrVcs.IMPORTANT_ERROR_NOTIFICATION, title, message, NotificationType.ERROR);
  }
  ...
  @Override
  public void showUnmergedFilesNotification(@NotNull final String operationName, @NotNull final Collection<BzrRepository> repositories) {
    ...
    Notificator.getInstance(myProject).notify(BzrVcs.IMPORTANT_ERROR_NOTIFICATION, title, description, NotificationType.ERROR,
      new NotificationListener() {
        ...
        public void hyperlinkUpdate(@NotNull Notification notification,
          ...
          if (event.getEventType() == HyperlinkEvent.EventType.ACTIVATED && event.getDescription().equals("resolve")) {
            ...
            new BzrConflictResolver(myProject, myBzr, myFacade, BzrUtil.getRootsFromRepositories(repositories), params).merge();
          }
        }
      }
    );
  ...
  public int showSmartOperationDialog(@NotNull Project project, @NotNull List<Change> changes, @NotNull String operation, boolean force) {
    return BzrSmartOperationDialog.showAndGetAnswer(myProject, changes, operation, true);
  }

  @Override
  public boolean showBranchIsNotFullyMergedDialog(@NotNull Project project, @NotNull final Map<BzrRepository, List<BzrCommit>> history,
    ...
    UIUtil.invokeAndWaitIfNeeded(new Runnable() {
      @Override
      public void run() {
        forceDelete.set(BzrBranchIsNotFullyMergedDialog.showAndGetAnswer(myProject, history, unmergedBranch, mergedToBranches, baseBranch));
      }
    });
