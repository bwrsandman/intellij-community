 ...
 */
public class BzrCommittedChangeListProvider implements CommittedChangesProvider<CommittedChangeList, ChangeBrowserSettings> {

  private static final Logger LOG = Logger.getInstance(BzrCommittedChangeListProvider.class);

  @NotNull private final Project myProject;

  public BzrCommittedChangeListProvider(@NotNull Project project) {
    myProject = project;
  }
  ...
  public ChangesBrowserSettingsEditor<ChangeBrowserSettings> createFilterUI(boolean showDateFilter) {
    return new BzrVersionFilterComponent(showDateFilter);
  }

  public RepositoryLocation getLocationFor(@NotNull FilePath root) {
    VirtualFile bzrRoot = BzrUtil.getBzrRootOrNull(root);
    if (bzrRoot == null) {
      return null;
    }
    BzrRepository repository = BzrUtil.getRepositoryManager(myProject).getRepositoryForRoot(bzrRoot);
    if (repository == null) {
      LOG.info("No BzrRepository for " + bzrRoot);
      return null;
    }
  ...
  public void loadCommittedChanges(ChangeBrowserSettings settings, RepositoryLocation location, int maxCount,
    ...
    try {
      getCommittedChangesImpl(settings, location, maxCount, new Consumer<BzrCommittedChangeList>() {
        @Override
        public void consume(BzrCommittedChangeList bzrCommittedChangeList) {
          consumer.consume(bzrCommittedChangeList);
        }
      });
    }
    finally {
  ...
  private void getCommittedChangesImpl(ChangeBrowserSettings settings, RepositoryLocation location, final int maxCount,
                                                            final Consumer<BzrCommittedChangeList> consumer)
    throws VcsException {
    BzrRepositoryLocation l = (BzrRepositoryLocation)location;
    ...
    BzrUtil.getLocalCommittedChanges(myProject, root, new Consumer<BzrSimpleHandler>() {
      public void consume(BzrSimpleHandler h) {
        if (!StringUtil.isEmpty(author)) {
          h.addParameters("--author=" + author);
        }
        if (beforeDate != null) {
          h.addParameters("--before=" + BzrUtil.bzrTime(beforeDate));
        }
        if (afterDate != null) {
          h.addParameters("--after=" + BzrUtil.bzrTime(afterDate));
        }
        if (maxCount != getUnlimitedCountValue()) {
          h.addParameters("-n" + maxCount);
        }
        if (beforeRev != null && afterRev != null) {
          h.addParameters(BzrUtil.formatLongRev(afterRev) + ".." + BzrUtil.formatLongRev(beforeRev));
        }
        else if (beforeRev != null) {
          h.addParameters(BzrUtil.formatLongRev(beforeRev));
        }
        else if (afterRev != null) {
          h.addParameters(BzrUtil.formatLongRev(afterRev) + "..");
        }
      }
    }, consumer, false);
  }
  ...
  public Pair<CommittedChangeList, FilePath> getOneList(final VirtualFile file, final VcsRevisionNumber number) throws VcsException {
    ...
    final List<BzrHeavyCommit> gitCommits =
      BzrHistoryUtils.commitsDetails(myProject, filePath, new SymbolicRefs(), Collections.singletonList(number.asString()));
    if (gitCommits.size() != 1) {
      return null;
    }
    final BzrHeavyCommit gitCommit = gitCommits.get(0);
    CommittedChangeList commit = new BzrCommittedChangeList(gitCommit.getDescription() + " (" + gitCommit.getShortHash().getString() + ")",
                                                            gitCommit.getDescription(), gitCommit.getAuthor(), (BzrRevisionNumber)number,
    ...
    final String afterTime = "--after=" + BzrUtil.bzrTime(gitCommit.getDate());
    final List<VcsFileRevision> history = BzrHistoryUtils.history(myProject, filePath, null, afterTime);
    if (history.isEmpty()) {
      return new Pair<CommittedChangeList, FilePath>(commit, filePath);
    }
    return new Pair<CommittedChangeList, FilePath>(commit, ((BzrFileRevision) history.get(history.size() - 1)).getPath());
  }
