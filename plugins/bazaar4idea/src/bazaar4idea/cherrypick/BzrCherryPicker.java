...
import bazaar4idea.commands.BzrUntrackedFilesOverwrittenByOperationDetector;
...
import static bazaar4idea.commands.BzrSimpleEventDetector.Event.CHERRY_PICK_CONFLICT;
import static bazaar4idea.commands.BzrSimpleEventDetector.Event.LOCAL_CHANGES_OVERWRITTEN_BY_CHERRY_PICK;

public class BzrCherryPicker {
  ...
  private static final Logger LOG = Logger.getInstance(BzrCherryPicker.class);
  ...
  public BzrCherryPicker(@NotNull Project project, @NotNull Bzr bzr, @NotNull BzrPlatformFacade platformFacade, boolean autoCommit) {
    myProject = project;
    myBzr = bzr;
  ...
  public void cherryPick(@NotNull Map<BzrRepository, List<VcsFullCommitDetails>> commitsInRoots) {
    List<BzrCommitWrapper> successfulCommits = new ArrayList<BzrCommitWrapper>();
    for (Map.Entry<BzrRepository, List<VcsFullCommitDetails>> entry : commitsInRoots.entrySet()) {
  ...
  private boolean cherryPick(@NotNull BzrRepository repository, @NotNull List<VcsFullCommitDetails> commits,
                             @NotNull List<BzrCommitWrapper> successfulCommits) {
    for (VcsFullCommitDetails commit : commits) {
      BzrSimpleEventDetector conflictDetector = new BzrSimpleEventDetector(CHERRY_PICK_CONFLICT);
      BzrSimpleEventDetector localChangesOverwrittenDetector = new BzrSimpleEventDetector(LOCAL_CHANGES_OVERWRITTEN_BY_CHERRY_PICK);
      BzrUntrackedFilesOverwrittenByOperationDetector untrackedFilesDetector =
        new BzrUntrackedFilesOverwrittenByOperationDetector(repository.getRoot());
      BzrCommandResult result = myBzr.cherryPick(repository, commit.getHash().asString(), myAutoCommit,
                                                 conflictDetector, localChangesOverwrittenDetector, untrackedFilesDetector);
      BzrCommitWrapper commitWrapper = new BzrCommitWrapper(commit);
      if (result.success()) {
        if (myAutoCommit) {
        ...
        boolean mergeCompleted = new CherryPickConflictResolver(myProject, myBzr, myPlatformFacade, repository.getRoot(),
  ...
  private boolean updateChangeListManagerShowCommitDialogAndRemoveChangeListOnSuccess(@NotNull BzrRepository repository,
                                                                                      @NotNull BzrCommitWrapper commit,
                                                                                      @NotNull List<BzrCommitWrapper> successfulCommits) {
  ...
  private void notifyConflictWarning(@NotNull BzrRepository repository, @NotNull BzrCommitWrapper commit,
                                     @NotNull List<BzrCommitWrapper> successfulCommits) {
    NotificationListener resolveLinkListener = new ResolveLinkListener(myProject, myBzr, myPlatformFacade, repository.getRoot(),
  ...
  private void notifyCommitCancelled(@NotNull BzrCommitWrapper commit, @NotNull List<BzrCommitWrapper> successfulCommits) {
  ...
  private boolean showCommitDialogAndWaitForCommit(@NotNull final BzrRepository repository, @NotNull final BzrCommitWrapper commit,
   ...
   */
  private void cancelCherryPick(@NotNull BzrRepository repository) {
    if (myAutoCommit) {
      removeCherryPickHead(repository);
    }
  }

  private void removeCherryPickHead(@NotNull BzrRepository repository) {
    File cherryPickHeadFile = new File(repository.getBzrDir().getPath(), CHERRY_PICK_HEAD_FILE);
  ...
  private void notifyError(@NotNull String content, @NotNull BzrCommitWrapper failedCommit, @NotNull List<BzrCommitWrapper> successfulCommits) {
  ...
  @NotNull
  private static String getSuccessfulCommitDetailsIfAny(@NotNull List<BzrCommitWrapper> successfulCommits) {
    String description = "";
    if (!successfulCommits.isEmpty()) {
  ...
  private void notifySuccess(@NotNull List<BzrCommitWrapper> successfulCommits) {
  ...
  @NotNull
  private static String getCommitsDetails(@NotNull List<BzrCommitWrapper> successfulCommits) {
    String description = "";
    for (BzrCommitWrapper commit : successfulCommits) {
  ...
  @NotNull
  private static String commitDetails(@NotNull BzrCommitWrapper commit) {
  ...
  private static class CherryPickConflictResolver extends BzrConflictResolver {

    public CherryPickConflictResolver(@NotNull Project project, @NotNull Bzr bzr, @NotNull BzrPlatformFacade facade, @NotNull VirtualFile root,
                                      @NotNull String commitHash, @NotNull String commitAuthor, @NotNull String commitMessage) {
      super(project, bzr, facade, Collections.singleton(root), makeParams(commitHash, commitAuthor, commitMessage));
    }
  ...
  private static class ResolveLinkListener implements NotificationListener {
    ...
    public ResolveLinkListener(@NotNull Project project, @NotNull Bzr bzr, @NotNull BzrPlatformFacade facade, @NotNull VirtualFile root,
                               @NotNull String commitHash, @NotNull String commitAuthor, @NotNull String commitMessage) {
    ...
    public void hyperlinkUpdate(@NotNull Notification notification,
      ...
      if (event.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
        if (event.getDescription().equals("resolve")) {
          new CherryPickConflictResolver(myProject, myBzr, myFacade, myRoot, myHash, myAuthor, myMessage).mergeNoProceed();
        }
      }
    }
  }
  ...
  /**
   * This class is needed to hold both the original BzrCommit, and the commit message which could be changed by the user.
   * Only the subject of the commit message is needed.
   */
  private static class BzrCommitWrapper {
    @NotNull private final VcsFullCommitDetails myOriginalCommit;
    @NotNull private String myActualSubject;

    private BzrCommitWrapper(@NotNull VcsFullCommitDetails commit) {
