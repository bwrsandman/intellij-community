 ...
 */
public class BzrBinaryHandler extends BzrHandler {
  /**
   * The logger
   */
  private static final Logger LOG = Logger.getInstance(BzrBinaryHandler.class.getName());
  /**
   * Stdout stream
   */
   ...
   */
  protected BzrBinaryHandler(@NotNull Project project, @NotNull File directory, @NotNull BzrCommand command) {
   ...
   * @param command a command to execute (if empty string, the parameter is ignored)
   */
  public BzrBinaryHandler(final Project project, final VirtualFile vcsRoot, final BzrCommand command) {
  ...
  public byte[] run() throws VcsException {
    addListener(new BzrHandlerListener() {
      ...
      public void processTerminated(int exitCode) {
        if (exitCode != 0 && !isIgnoredErrorCode(exitCode)) {
          ...
          cs = cs == null ? BzrUtil.UTF8_CHARSET : cs;
          ...
          if (message.length() == 0) {
            ...
            if (!isStderrSuppressed()) {
              BzrVcs.getInstance(myProject).showErrorMessages(message);
            }
          }
          if (message != null) {
            //noinspection ThrowableInstanceNeverThrown
    ...
    BzrHandlerUtil.runInCurrentThread(this, null);
    //noinspection ThrowableResultOfMethodCallIgnored
    if (myException.get() != null) {
