...
import org.jetbrains.bazaar4idea.ssh.BzrSSHHandler;
import org.jetbrains.bazaar4idea.ssh.BzrXmlRpcSshService;
...
public abstract class BzrHandler {
  ...
  protected final BzrCommand myCommand;
  ...
  protected static final Logger LOG = Logger.getInstance(BzrHandler.class.getName());
  ...
  private BzrVcsSettings myProjectSettings;
   ...
   */
  protected BzrHandler(@NotNull Project project, @NotNull File directory, @NotNull BzrCommand command) {
    myProject = project;
    myCommand = command;
    myAppSettings = BzrVcsApplicationSettings.getInstance();
    myProjectSettings = BzrVcsSettings.getInstance(myProject);
    myEnv = new HashMap<String, String>(System.getenv());
    myVcs = BzrVcs.getInstance(project);
    myWorkingDirectory = directory;
    myCommandLine = new GeneralCommandLine();
    if (myAppSettings != null) {
      myCommandLine.setExePath(myAppSettings.getPathToBzr());
    }
   ...
   */
  protected BzrHandler(final Project project, final VirtualFile vcsRoot, final BzrCommand command) {
  ...
  private boolean isCmd() {
    return myAppSettings.getPathToBzr().toLowerCase().endsWith("cmd");
  }
  ...
  public synchronized void start() {
    ...
    try {
      ...
      // setup environment
      BzrRemoteProtocol remoteProtocol = BzrRemoteProtocol.fromUrl(myUrl);
      if (remoteProtocol == BzrRemoteProtocol.SSH && myProjectSettings.isIdeaSsh()) {
        BzrXmlRpcSshService ssh = ServiceManager.getService(BzrXmlRpcSshService.class);
        myEnv.put(BzrSSHHandler.BAZAAR_SSH_ENV, ssh.getScriptPath().getPath());
        myHandlerNo = ssh.registerHandler(new BzrSSHGUIHandler(myProject, myState));
        myEnvironmentCleanedUp = false;
        myEnv.put(BzrSSHHandler.SSH_HANDLER_ENV, Integer.toString(myHandlerNo));
        int port = ssh.getXmlRcpPort();
        myEnv.put(BzrSSHHandler.SSH_PORT_ENV, Integer.toString(port));
        LOG.debug(String.format("handler=%s, port=%s", myHandlerNo, port));
      }
      else if (remoteProtocol == BzrRemoteProtocol.HTTP) {
        BzrHttpAuthService service = ServiceManager.getService(BzrHttpAuthService.class);
        myEnv.put(BzrAskPassXmlRpcHandler.BAZAAR_ASK_PASS_ENV, service.getScriptPath().getPath());
        assert myUrl != null : "myUrl can't be null here";
        BzrHttpAuthenticator httpAuthenticator = service.createAuthenticator(myProject, myState, myCommand, myUrl);
        myHandlerNo = service.registerHandler(httpAuthenticator);
        myEnvironmentCleanedUp = false;
        myEnv.put(BzrAskPassXmlRpcHandler.BAZAAR_ASK_PASS_HANDLER_ENV, Integer.toString(myHandlerNo));
        int port = service.getXmlRcpPort();
        myEnv.put(BzrAskPassXmlRpcHandler.BAZAAR_ASK_PASS_PORT_ENV, Integer.toString(port));
        LOG.debug(String.format("handler=%s, port=%s", myHandlerNo, port));
  ...
  private void addAuthListener(@NotNull final BzrHttpAuthenticator authenticator) {
    // TODO this code should be located in BzrLineHandler, and the other remote code should be move there as well
    if (this instanceof BzrLineHandler) {
      ((BzrLineHandler)this).addLineListener(new BzrLineHandlerAdapter() {
  ...
  protected synchronized void cleanupEnv() {
    ...
    BzrRemoteProtocol remoteProtocol = BzrRemoteProtocol.fromUrl(myUrl);
    if (remoteProtocol == BzrRemoteProtocol.SSH) {
      BzrXmlRpcSshService ssh = ServiceManager.getService(BzrXmlRpcSshService.class);
