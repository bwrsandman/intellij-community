...
/**
 * <p>Handles "ask username" and "ask password" requests from Bazaar:
 ...
 * <p>If no username is specified in the URL, Bazaar queries for the username and for the password consecutively.
 *    In this case to avoid showing dialogs twice, the component asks for both credentials at once,
 *    and remembers the password to provide it to the Bazaar process during the next request without requiring user interaction.</p>
 * <p>New instance of the BzrAskPassGuiHandler should be created for each session, i. e. for each remote operation call.</p>
 *
 * @author Kirill Likhodedov
 */
class BzrHttpGuiAuthenticator implements BzrHttpAuthenticator {

  private static final Logger LOG = Logger.getInstance(BzrHttpGuiAuthenticator.class);
  private static final Class<BzrHttpAuthenticator> PASS_REQUESTER = BzrHttpAuthenticator.class;
  ...
  BzrHttpGuiAuthenticator(@NotNull Project project, @Nullable ModalityState modalityState, @NotNull BzrCommand command, @NotNull String url) {
    myProject = project;
    myModalityState = modalityState;
    myTitle = "Bazaar " + StringUtil.capitalize(command.name());
  ...
  public void saveAuthData() {
    ...
    if (myUrl != null && myLogin != null) {
      BzrRememberedInputs.getInstance().addUrl(myUrl, myLogin);
    }
  ...
  private String adjustUrl(@NotNull String url) {
    if (StringUtil.isEmptyOrSpaces(url)) {
      // if Bazaar doesn't specify the URL in the username/password query, we use the url from the Bazaar command
