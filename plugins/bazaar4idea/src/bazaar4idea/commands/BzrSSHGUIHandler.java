...
import bazaar4idea.config.SSHConnectionSettings;
...
public class BzrSSHGUIHandler {
   ...
   */
  BzrSSHGUIHandler(@Nullable Project project, @Nullable ModalityState state) {
    myProject = project;
    myState = state;
  }
  ...
  public boolean verifyServerHostKey(final String hostname,
    ...
    if (isNew) {
      message = BzrBundle.message("ssh.new.host.key", hostname, port, fingerprint, serverHostKeyAlgorithm);
    }
    else {
      message = BzrBundle.message("ssh.changed.host.key", hostname, port, fingerprint, serverHostKeyAlgorithm);
    }
    final AtomicBoolean rc = new AtomicBoolean();
    UIUtil.invokeAndWaitIfNeeded(new Runnable() {
      public void run() {
        rc.set(Messages.YES == Messages.showYesNoDialog(myProject, message, BzrBundle.getString("ssh.confirm.key.titile"), null));
      }
    });
    return rc.get();
  }
  ...
  public String askPassphrase(final String username, final String keyPath, boolean resetPassword, final String lastError) {
    ...
    return PasswordSafePromptDialog.askPassphrase(myProject, myState, BzrBundle.getString("ssh.ask.passphrase.title"),
                                                  BzrBundle.message("ssh.askPassphrase.message", keyPath, username),
                                                  BzrSSHGUIHandler.class, "PASSPHRASE:" + keyPath, resetPassword, error
    );
  }
  ...
  private void showError(final String lastError) {
    if (lastError.length() != 0) {
      Messages.showErrorDialog(myProject, lastError, BzrBundle.getString("ssh.error.title"));
    }
  }
  ...
  public Vector<String> replyToChallenge(final String username,
    ...
    try {
      EventQueue.invokeAndWait(new Runnable() {
        public void run() {
          showError(lastError);
          BzrSSHKeyboardInteractiveDialog dialog =
            new BzrSSHKeyboardInteractiveDialog(name, numPrompts, instruction, prompt, echo, username);
          dialog.show();
          if (dialog.isOK()) {
  ...
  public String askPassword(final String username, boolean resetPassword, final String lastError) {
    String error = processLastError(resetPassword, lastError);
    return PasswordSafePromptDialog.askPassword(myProject, myState, BzrBundle.getString("ssh.password.title"),
                                                BzrBundle.message("ssh.password.message", username),
                                                BzrSSHGUIHandler.class, "PASSWORD:" + username, resetPassword, error);
  }
  ...
  private class BzrSSHKeyboardInteractiveDialog extends DialogWrapper {
    /**
     * input fields
     */
    ...
    public BzrSSHKeyboardInteractiveDialog(String name,
                                           final int numPrompts,
                                           final String instruction,
                                           final Vector<String> prompt,
                                           final Vector<Boolean> echo,
      ...
      myEcho = echo;
      myUserName = userName;
      setTitle(BzrBundle.message("ssh.keyboard.interactive.title", name));
      init();
      setResizable(true);
      setModal(true);
    }
    ...
    protected JComponent createCenterPanel() {
      if (contents == null) {
        ...
        c.gridx = 0;
        c.gridy = line;
        contents.add(new JLabel(BzrBundle.getString("ssh.keyboard.interactive.username")), c);
        c = new GridBagConstraints();
        c.insets = insets;
        c.gridx = 1;
        c.gridy = line;
