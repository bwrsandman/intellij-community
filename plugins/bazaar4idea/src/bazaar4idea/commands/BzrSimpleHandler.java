...
/**
 * Simple Bazaar handler that accumulates stdout and stderr and has nothing on stdin.
 ...
 * simple commands.
 */
public class BzrSimpleHandler extends BzrTextHandler {
  /**
   * Stderr output
   */
  ...
  public BzrSimpleHandler(@NotNull Project project, @NotNull File directory, @NotNull BzrCommand command) {
   ...
   */
  @SuppressWarnings({"WeakerAccess"})
  public BzrSimpleHandler(@NotNull final Project project, @NotNull final VirtualFile directory, @NotNull final BzrCommand command) {
    super(project, directory, command);
  }
  ...
  /**
   * Execute without UI. If UI interactions are required (for example SSH popups or progress dialog), use {@link BzrHandlerUtil} methods.
   *
  ...
  public String run() throws VcsException {
    ...
    addListener(new BzrHandlerListener() {
      public void processTerminated(final int exitCode) {
        try {
          if (exitCode == 0 || isIgnoredErrorCode(exitCode)) {
            ...
            if (msg.length() == 0) {
              msg = BzrBundle.message("git.error.exit", exitCode);
            }
