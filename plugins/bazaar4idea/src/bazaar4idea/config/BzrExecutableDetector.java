...
/**
 * Tries to detect the path to Bazaar executable.
 *
 * @author Kirill Likhodedov
 */
public class BzrExecutableDetector {

  private static final Logger LOG = Logger.getInstance(BzrExecutableDetector.class);
  private static final String[] UNIX_PATHS = { "/usr/local/bin",
                                               "/usr/bin",
                                               "/opt/local/bin",
                                               "/opt/bin",
                                               "/usr/local/bzr/bin"};
  private static final String UNIX_EXECUTABLE = "bzr";
  ...
  private static final String BZR_CMD = "bzr.cmd";
  private static final String BZR_EXE = "bzr.exe";

  public static final String DEFAULT_WIN_BZR = BZR_EXE;
  ...
  /**
   * Looks into the %PATH% and checks Bazaar directories mentioned there.
   * @return Bazaar executable to be used or null if nothing interesting was found in the PATH.
   */
  @Nullable
  ...
  private static boolean looksLikeBzr(@NotNull String path) {
    List<String> dirs = FileUtil.splitPath(path);
    for (String dir : dirs) {
      if (dir.toLowerCase().startsWith("bzr")) {
        return true;
      }
    }
    return false;
  ...
  private static String checkBinDir(@NotNull File binDir) {
    ...
    for (String exec : new String[]{BZR_CMD, BZR_EXE}) {
  ...
  // Compare strategy: greater is better (if v1 > v2, then v1 is a better candidate for the Bazaar executable)
  private static class VersionDirsComparator implements Comparator<File> {
    ...
    public int compare(File f1, File f2) {
      ...
      // C:\Program Files\Bazaar is better candidate for _default_ than C:\Program Files\Bazaar_1.8.0
      if (name1.equals("bzr")) {
        return name2.equals("bzr") ? fallback(f1, f2) : 1;
      }
      else if (name2.equals("bzr")) {
        return -1;
      }

      final Pattern BAZAAR_WITH_VERSION = Pattern.compile("^bzr[ _]*([\\d\\.]*).*$");
      Matcher m1 = BAZAAR_WITH_VERSION.matcher(name1);
      Matcher m2 = BAZAAR_WITH_VERSION.matcher(name2);
      if (m1.matches() && m2.matches()) {
        BzrVersion v1 = parseBzrVersion(m1.group(1));
        BzrVersion v2 = parseBzrVersion(m2.group(1));
        if (v1 == null || v2 == null) {
    ...
    private static int fallback(@NotNull File f1, @NotNull File f2) {
      ...
      // probably some unrecognized format of Bazaar directory naming => just compare lexicographically
      String name1 = f1.getName().toLowerCase();
      String name2 = f2.getName().toLowerCase();
      return name1.compareTo(name2);
    }

    // not using BzrVersion#parse(), because it requires at least 3 items in the version (1.7.3),
    // and parses the `bzr version` command output, not just the version string.
    @Nullable
    private static BzrVersion parseBzrVersion(@Nullable String name) {
      if (name == null) {
        return null;
      }
