...
import bazaar4idea.history.browser.SymbolicRefs;
import bazaar4idea.history.browser.SymbolicRefsI;
...
/**
 * A collection of methods for retrieving history information from native Bazaar.
 */
public class BzrHistoryUtils {

  /**
   * A parameter to {@code bzr log} which is equivalent to {@code --all}, but doesn't show the stuff from index or stash.
   */
  public static List<String> LOG_ALL = Arrays.asList("HEAD", "--branches", "--remotes", "--tags");

  private final static Logger LOG = Logger.getInstance(BzrHistoryUtils.class);

  private BzrHistoryUtils() {
  }
  ...
  public static long getHeadTs(final Project project, FilePath filePath) throws VcsException {
    ...
    final BzrLogRecord record = parser.parseOneRecord(result);
    if (record == null) {
      return -1;
    }
  ...
  public static VcsRevisionNumber getCurrentRevision(@NotNull Project project, @NotNull FilePath filePath,
    ...
    record.setUsedHandler(h);
    return new BzrRevisionNumber(record.getHash(), record.getDate());
  }
  ...
  public static VcsRevisionDescription getCurrentRevisionDescription(final Project project, FilePath filePath, @Nullable String branch) throws VcsException {
    ...
    return new VcsRevisionDescriptionImpl(new BzrRevisionNumber(record.getHash(), record.getDate()), record.getDate(), author,
  ...
  public static ItemLatestState getLastRevision(final Project project, FilePath filePath) throws VcsException {
    VirtualFile root = BzrUtil.getBzrRoot(filePath);
    BzrBranch c = BzrBranchUtil.getCurrentBranch(project, root);
    BzrBranch t = c == null ? null : BzrBranchUtil.tracked(project, root, c.getName());
    if (t == null) {
    ...
    BzrLogRecord record = parser.parseOneRecord(result);
    if (record == null) {
      return null;
    }
    ...
    record.setUsedHandler(h);
    return new ItemLatestState(new BzrRevisionNumber(record.getHash(), record.getDate()), exists, false);
  }
  ...
  public static void dumpFullHistory(final Project project, VirtualFile root, final String outFilePath) throws VcsException {
    ...
    try {
      ...
      h.addLineListener(new BzrLineHandlerListener() {
        @Override
        public void onLineAvailable(String line, Key outputType) {
          if (line.length() == 0) return;
          try {
            BzrCommitsSequentialIndex.parseRecord(line);
            stream[0].write((line + '\n').getBytes("UTF-8"));
          }
          catch (IOException e) {
   ...
   * @param root              Bazaar root - optional: if this is null, then bzr root will be detected automatically.
   ...
   */
  public static void history(final Project project, FilePath path, @Nullable VirtualFile root, final Consumer<BzrFileRevision> consumer,
    ...
    try {
      finalRoot = (root == null ? BzrUtil.getBzrRoot(filePath) : root);
    }
    catch (VcsException e) {
      exceptionConsumer.consume(e);
      return;
    }
    final BzrLogParser logParser = new BzrLogParser(project, BzrLogParser.NameStatus.STATUS,
    ...
    final Consumer<BzrLogRecord> resultAdapter = new Consumer<BzrLogRecord>() {
      public void consume(BzrLogRecord record) {
        ...
        final BzrRevisionNumber revision = new BzrRevisionNumber(record.getHash(), record.getDate());
        ...
        try {
          ...
          consumer.consume(new BzrFileRevision(project, revisionPath, revision, Pair.create(authorPair, committerPair), message, null,
                                               new Date(record.getAuthorTimeStamp()), parents));
          List<BzrLogStatusInfo> statusInfos = record.getStatusInfos();
          if (statusInfos.isEmpty()) {
            // can safely be empty, for example, for simple merge commits that don't change anything.
            return;
          }
          if (statusInfos.get(0).getType() == BzrChangeType.ADDED && !filePath.isDirectory()) {
    ...
    while (currentPath.get() != null && firstCommitParent.get() != null) {
      ...
      logHandler.get().addLineListener(new BzrLineHandlerAdapter() {
        @Override
        public void onLineAvailable(String line, Key outputType) {
          final BzrLogRecord record = accumulator.acceptLine(line);
          if (record != null) {
        ...
        public void processTerminated(int exitCode) {
          try {
            ...
            final BzrLogRecord record = accumulator.processLast();
            if (record != null) {
              resultAdapter.consume(record);
            }
          } 
  ...
  public static List<? extends VcsShortCommitDetails> readAllMiniDetails(final Project project, final VirtualFile root) throws VcsException {
    BzrSimpleHandler h = new BzrSimpleHandler(project, root, BzrCommand.LOG);
    BzrLogParser parser = new BzrLogParser(project, BzrLogParser.NameStatus.NONE, HASH, PARENTS, AUTHOR_NAME,
    ...
    List<BzrLogRecord> records = parser.parse(output);

    return ContainerUtil.mapNotNull(records, new Function<BzrLogRecord, VcsShortCommitDetails>() {
      @Override
      public VcsShortCommitDetails fun(BzrLogRecord record) {
  ...
  public static List<? extends VcsShortCommitDetails> readMiniDetails(final Project project, final VirtualFile root, List<String> hashes) throws VcsException {
    ...
    return ContainerUtil.map(records, new Function<BzrLogRecord, VcsShortCommitDetails>() {
      @Override
      public VcsShortCommitDetails fun(BzrLogRecord record) {
        List<Hash> parents = new SmartList<Hash>();
        for (String parent : record.getParentsHashes()) {
  ...
  public static List<TimedVcsCommit> readAllHashes(@NotNull final Project project,
    ...
    BzrLineHandler h = new BzrLineHandler(project, root, BzrCommand.LOG);
    final BzrLogParser parser = new BzrLogParser(project, BzrLogParser.NameStatus.NONE, HASH, PARENTS, COMMIT_TIME,
    ...
    h.addLineListener(new BzrLineHandlerListener() {
      ...
      public void onLineAvailable(String line, Key outputType) {
        try {
          int recordEnd = line.indexOf(BzrLogParser.RECORD_END);
  ...
  private static List<TimedVcsCommit> parseCommit(final Project project, BzrLogParser parser, StringBuilder record,
                                                  final Consumer<VcsUser> userRegistry) {
    List<BzrLogRecord> rec = parser.parse(record.toString());
    return ContainerUtil.mapNotNull(rec, new Function<BzrLogRecord, TimedVcsCommit>() {
      @Override
      public TimedVcsCommit fun(BzrLogRecord record) {
        if (record == null) {
          return null;
        }
  ...
  @NotNull
  private static TimedVcsCommit convert(Project project, BzrLogRecord rec) {
  ...
  private static class MyTokenAccumulator {
    ...
    public MyTokenAccumulator(BzrLogParser parser) {
      myParser = parser;
    }

    @Nullable
    public BzrLogRecord acceptLine(String s) {
      final boolean lineEnd = s.startsWith(BzrLogParser.RECORD_START);
      if (lineEnd && (!myNotStarted)) {
        final String line = myBuffer.toString();
        myBuffer.setLength(0);
        myBuffer.append(s.substring(BzrLogParser.RECORD_START.length()));

        return processResult(line);
      }
      else {
        myBuffer.append(lineEnd ? s.substring(BzrLogParser.RECORD_START.length()) : s);
        myBuffer.append("\n");
      }
      myNotStarted = false;
    ...
    public BzrLogRecord processLast() {
      return processResult(myBuffer.toString());
    }

    private BzrLogRecord processResult(final String line) {
  ...
  public static List<VcsFileRevision> history(final Project project, FilePath path, final VirtualFile root, final String... parameters) throws VcsException {
    ...
    history(project, path, root, new Consumer<BzrFileRevision>() {
      @Override public void consume(BzrFileRevision bzrFileRevision) {
        rc.add(bzrFileRevision);
      }
  ...
  public static List<Pair<SHAHash, Date>> onlyHashesHistory(Project project, FilePath path, final VirtualFile root, final String... parameters)
    ...
    for (BzrLogRecord record : parser.parse(output)) {
      record.setUsedHandler(h);
   ...
   */
  @NotNull
  public static List<BzrCommit> history(@NotNull final Project project, @NotNull final VirtualFile root, String... parameters)
                                        throws VcsException {
    BzrSimpleHandler h = new BzrSimpleHandler(project, root, BzrCommand.LOG);
    BzrLogParser parser = new BzrLogParser(project, BzrLogParser.NameStatus.STATUS, HASH, COMMIT_TIME, AUTHOR_NAME, AUTHOR_TIME,
    ...
    return ContainerUtil.mapNotNull(records, new Function<BzrLogRecord, BzrCommit>() {
      @Override
      public BzrCommit fun(BzrLogRecord record) {
        try {
  ...
  private static BzrCommit createCommit(@NotNull Project project, @NotNull VirtualFile root, @NotNull BzrLogRecord record)
                                        throws VcsException {
    ...
    VcsLogObjectsFactory factory = vcsObjectsFactory(project);
    return new BzrCommit(project, HashImpl.build(record.getHash()), parents, record.getCommitTime(), root, record.getSubject(),
   ...
   *    returned by Bazaar.</p>
   * <p>This method is not efficient.
   *    Consider using {@link #history(Project, VirtualFile, String...)} if you can afford storing the whole Bazaar output in memory while
   *    parsing.</p>
   */
  public static void historyWithLinks(final Project project,
                                      FilePath path,
                                      @Nullable final SymbolicRefsI refs,
                                      @NotNull final AsynchConsumer<BzrHeavyCommit> gitCommitConsumer,
    ...
    final BzrLineHandler h = new BzrLineHandler(project, root, BzrCommand.LOG);
    final BzrLogParser
      parser = new BzrLogParser(project, BzrLogParser.NameStatus.STATUS, HASH, HASH, COMMIT_TIME, AUTHOR_NAME, AUTHOR_TIME, AUTHOR_EMAIL,
    ...
    h.addLineListener(new BzrLineHandlerAdapter() {
      @Override
  ...
  private static void takeLine(final Project project, String line,
                               ...
                               SymbolicRefsI refs,
                               VirtualFile root,
                               VcsException[] exc, BzrLineHandler h, AsynchConsumer<BzrHeavyCommit> gitCommitConsumer) {
    final String text = sb.toString();
    sb.setLength(0);
    sb.append(line);
    if (text.length() == 0) return;
    BzrLogRecord record = parser.parseOneRecord(text);

    final BzrHeavyCommit gitCommit;
    try {
  ...
  @NotNull
  private static BzrHeavyCommit createCommit(@NotNull Project project, @Nullable SymbolicRefsI refs, @NotNull VirtualFile root,
                                        @NotNull BzrLogRecord record) throws VcsException {
    ...
    BzrHeavyCommit
      gitCommit = new BzrHeavyCommit(root, AbstractHash.create(record.getHash()), new SHAHash(record.getHash()), record.getAuthorName(),
  ...
  public static Pair<AbstractHash, AbstractHash> getStashTop(@NotNull Project project, @NotNull VirtualFile root) throws VcsException {
    ...
    String out;
    h.setCharset(Charset.forName(BzrConfigUtil.getLogEncoding(project, root)));
    out = h.run();
    final List<BzrLogRecord> bzrLogRecords = parser.parse(out);
    for (BzrLogRecord bzrLogRecord : bzrLogRecords) {
      ...
      String out1;
      out1 = h1.run();
      final List<BzrLogRecord> bzrLogRecords1 = parser1.parse(out1);
      LOG.assertTrue(bzrLogRecords1.size() == 1, String.format("bzrLogRecords size is incorrect. size: %s, records: %s, output: %s",
                                                               bzrLogRecords1.size(), bzrLogRecords1, out1));
      final BzrLogRecord logRecord = bzrLogRecords1.get(0);
      ...
      return new Pair<AbstractHash, AbstractHash>(AbstractHash.create(bzrLogRecord.getHash()), indexCommit == null ? null : AbstractHash.create(indexCommit));
    }
    return null;
  }

  @Nullable
  public static List<Pair<String, BzrHeavyCommit>> loadStashStackAsCommits(@NotNull Project project, @NotNull VirtualFile root,
    ...
    out = h.run();
    final List<BzrLogRecord> bzrLogRecords = parser.parse(out);
    final List<Pair<String, BzrHeavyCommit>> result = new ArrayList<Pair<String, BzrHeavyCommit>>();
    for (BzrLogRecord bzrLogRecord : bzrLogRecords) {
      ProgressManager.checkCanceled();
      final BzrHeavyCommit gitCommit = createCommit(project, refs, root, bzrLogRecord);
      result.add(new Pair<String, BzrHeavyCommit>(bzrLogRecord.getShortenedRefLog(), gitCommit));
    }
    return result;
  }
  ...
  @Deprecated
  @NotNull
  public static List<BzrHeavyCommit> commitsDetails(@NotNull Project project, @NotNull FilePath path, @Nullable SymbolicRefsI refs,
    ...
    String output = h.run();
    final List<BzrHeavyCommit> rc = new ArrayList<BzrHeavyCommit>();
    for (BzrLogRecord record : parser.parse(output)) {
      final BzrHeavyCommit gitCommit = createCommit(project, refs, root, record);
      rc.add(gitCommit);
    }
    return rc;
  }
  ...
  public static List<BzrCommit> commitsDetails(@NotNull Project project, @NotNull VirtualFile root,
    ...
    String output = h.run();
    final List<BzrCommit> rc = new ArrayList<BzrCommit>();
    for (BzrLogRecord record : parser.parse(output)) {
  ...
  @NotNull
  public static List<BzrCommit> getAllDetails(@NotNull Project project, @NotNull VirtualFile root,
  ...
  public static long getAuthorTime(Project project, FilePath path, final String commitsId) throws VcsException {
    ...
    String output = h.run();
    BzrLogRecord logRecord = parser.parseOneRecord(output);
  ...
  public static void hashesWithParents(Project project, FilePath path, final AsynchConsumer<CommitHashPlusParents> consumer,
    ...
    final VirtualFile root = BzrUtil.getBzrRoot(path);
    final BzrLineHandler h = new BzrLineHandler(project, root, BzrCommand.LOG);
    final BzrLogParser parser = new BzrLogParser(project, BzrLogParser.NameStatus.NAME, HASH, COMMIT_TIME, PARENTS, AUTHOR_NAME);
    h.setStdoutSuppressed(true);
    h.addParameters(parameters);
      ...
      public void onLineAvailable(final String line, final Key outputType) {
        try {
          if (ProcessOutputTypes.STDOUT.equals(outputType)) {
            ...
            BzrLogRecord record = parser.parseOneRecord(line);
  ...
  public static FilePath getLastCommitName(@NotNull Project project, FilePath path) {
    ...
    if (change != null && change.getType() == Change.Type.MOVED) {
     // BzrContentRevision r = (BzrContentRevision)change.getBeforeRevision();
  ...
  @Nullable
  public static BzrRevisionNumber getMergeBase(final Project project, final VirtualFile root, @NotNull final String first,
    ...
    if (output.length() == 0) {
      ...
      return BzrRevisionNumber.resolve(project, root, output);
    }
  }
}
