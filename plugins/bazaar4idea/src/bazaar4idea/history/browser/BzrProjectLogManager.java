...
import bazaar4idea.history.wholeTree.BzrLog;
import bazaar4idea.history.wholeTree.LogFactoryService;

import java.util.Arrays;
import java.util.List;
import java.util.concurrent.atomic.AtomicReference;

public class BzrProjectLogManager {
  private final static Logger LOG = Logger.getInstance(BzrProjectLogManager.class);
  ...
  private final AtomicReference<BzrLog> myLogRef;
  ...
  public BzrProjectLogManager(final Project project, final ProjectLevelVcsManager vcsManager, final LogFactoryService logFactoryService) {
    myProject = project;
    myVcsManager = vcsManager;
    myLogFactoryService = logFactoryService;
    myCurrentContent = new AtomicReference<Content>();
    myLogRef = new AtomicReference<BzrLog>();
  ...
  public static BzrProjectLogManager getInstance(final Project project) {
    return PeriodicalTasksCloser.getInstance().safeGetService(project, BzrProjectLogManager.class);
  }
  ...
  private void recalculateWindows() {
    ...
    final BzrLog bzrLog = myLogFactoryService.createComponent(true);
    final ContentFactory contentFactory = ContentFactory.SERVICE.getInstance();
    final Content content = contentFactory.createContent(bzrLog.getVisualComponent(), CONTENT_KEY, false);
    ...
    bzrLog.rootsChanged(fileList);
  }

  public BzrLog getLog() {
    return myLogRef.get();
  }
