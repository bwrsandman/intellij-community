...
public class ByRootLoader extends TaskDescriptor {
  ...
  @NotNull
  private final BzrLogFilters myBzrLogFilters;

  public ByRootLoader(Project project,
                      LoaderAndRefresherImpl.MyRootHolder rootHolder,
                      Mediator mediator,
                      DetailsCache detailsCache,
                      Ticket ticket, UsersIndex usersIndex, BzrLogFilters bzrLogFilters, final Collection<String> startingPoints) {
  ...
  private void reportStashHead() {
    ...
    try {
      myMediator.acceptStashHead(myTicket, myRootHolder.getRoot(), BzrHistoryUtils.getStashTop(myProject, myRootHolder.getRoot()));
    }
  ...
  private void loadStash() {
    ...
    final List<BzrHeavyCommit> details = new ArrayList<BzrHeavyCommit>();
    final List<CommitI> commits = new ArrayList<CommitI>();
    final Map<AbstractHash, String> stashMap = new HashMap<AbstractHash, String>();
    final List<List<AbstractHash>> parents = ! myBzrLogFilters.haveDisordering() ? new ArrayList<List<AbstractHash>>() : null;

    myBzrLogFilters.callConsumer(new Consumer<List<ChangesFilter.Filter>>() {
      @Override
      public void consume(List<ChangesFilter.Filter> filters) {
        ...
        try {
          ...
          final List<Pair<String,BzrHeavyCommit>> stash = BzrHistoryUtils
            .loadStashStackAsCommits(myProject, myRootHolder.getRoot(), mySymbolicRefs, parameters.toArray(new String[parameters.size()]));
          if (stash == null) return;
          for (Pair<String, BzrHeavyCommit> pair : stash) {
            ProgressManager.checkCanceled();
            final BzrHeavyCommit gitCommit = pair.getSecond();
  ...
  public void loadByHashesAside(final ContinuationContext context) {
    ...
    final List<String> hashes = myBzrLogFilters.getPossibleReferencies();
    ...
    myBzrLogFilters.callConsumer(new Consumer<List<ChangesFilter.Filter>>() {
      ...
      public void consume(List<ChangesFilter.Filter> filters) {
        for (String hash : hashes) {
          try {
            final SHAHash shaHash = BzrChangeUtils
              .commitExists(myProject, myRootHolder.getRoot(), hash, Collections.<VirtualFile>emptyList());
            ...
            if (myStartingPoints != null && (! myStartingPoints.isEmpty())) {
              ...
              for (String startingPoint : myStartingPoints) {
                if(BzrChangeUtils.isAnyLevelChild(myProject, myRootHolder.getRoot(), shaHash, startingPoint)) {
                  matches = true;
                  break;
                }
              }
            ...
            final List<BzrHeavyCommit> commits = myLowLevelAccess.getCommitDetails(Collections.singletonList(shaHash.getValue()), mySymbolicRefs);
            if (commits.isEmpty()) continue;
            assert commits.size() == 1;
            final BzrHeavyCommit commitDetails = commits.get(0);
            boolean isOk = true;
  ...
  private void appendCommits(List<CommitI> result, List<BzrHeavyCommit> commits) {
  ...
  private CommitI createCommitI(BzrHeavyCommit commit) {
    CommitI commitObj =
