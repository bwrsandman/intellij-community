 ...
 * Time: 7:33 PM
 */
public class BzrCommitsSequentialIndex implements BzrCommitsSequentially {
  ...
  private static final Logger LOG = Logger.getInstance(BzrCommitsSequentialIndex.class);

  public BzrCommitsSequentialIndex() {
    myLock = new Object();
    final File vcsFile = new File(PathManager.getSystemPath(), "vcs");
    myDir = new File(vcsFile, "bzr_line");
    myDir.mkdirs();
  ...
  private class LoadTask extends TaskDescriptor {
    ...
    private void loadImpl() throws VcsException {
      ...
      if (latestWrittenHash[0] != null) {
        ItemLatestState lastRevision = BzrHistoryUtils.getLastRevision(myProject, new FilePathImpl(myFile));
        if (lastRevision == null) {
          // no history at the moment
          return;
        }
        BzrRevisionNumber lastRevNo = (BzrRevisionNumber) lastRevision.getNumber();
        if (lastRevision.isItemExists() && lastRevNo.asString().equals(latestWrittenHash[0].getString())) {
          // no refresh needed
          return;
        }
    ...
    private void initHistory() throws VcsException {
      ...
      BzrHistoryUtils.dumpFullHistory(myProject, myFile, outFilePath);
    }
    ...
    private void appendHistory(final long since, final AbstractHash hash) throws VcsException {
      ...
      final List<Pair<SHAHash,Date>> pairs =
              BzrHistoryUtils.onlyHashesHistory(myProject, new FilePathImpl(myFile), "--all", "--date-order", "--full-history", "--sparse",
                                                "--after=" + (since / 1000));
