 ...
 *         Time: 2:27 PM
 */
public class BzrLogDetailsPanel {
  ...
  public void setData(VirtualFile root, @NotNull final BzrHeavyCommit commit) {
    myRoot = root;
    myCommit = commit;
    redrawBranchLabels();
  ...
  void redrawBranchLabels() {
    ...
    for (String s : myCommit.getRemoteBranches()) {
      myMarksPanel.add(new JLabel(new CaptionIcon(BzrLogUI.Colors.remote, font, s, myMarksPanel, CaptionIcon.Form.SQUARE, false,
                                       s.equals(remoteName))));
    }
    for (String s : myCommit.getTags()) {
      myMarksPanel.add(new JLabel(new CaptionIcon(BzrLogUI.Colors.tag, font, s, myMarksPanel, CaptionIcon.Form.ROUNDED, false, false)));
    }
  }
  ...
  private static class MyPresentationData {
    ...
    public void setCommit(final VirtualFile root, final BzrHeavyCommit c) {
    ...
    public void branchesPresentation() {
      final BzrLogSettings settings = BzrLogSettings.getInstance(myProject);
