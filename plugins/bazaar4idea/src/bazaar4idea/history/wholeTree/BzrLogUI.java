 ...
 * @author irengrig
 */
public class BzrLogUI implements Disposable {
  private final static Logger LOG = Logger.getInstance("#bazaar4idea.history.wholeTree.BzrLogUI");
  ...
  public static final String BZR_LOG_TABLE_PLACE = "bzr log table";
  ...
  private BzrTableScrollChangeListener myMyChangeListener;
  ...
  private GenericDetailsLoader<CommitI, BzrHeavyCommit> myDetailsLoader;
  private GenericDetailsLoader<CommitI, List<String>> myBranchesLoader;
  private BzrLogDetailsPanel myDetailsPanel;
  ...
  private final BzrLogUI.MyShowTreeAction myMyShowTreeAction;
  private final BzrLogShowOnlyStarredCommitsAction myFilterStarredAction;
  ...
  private BzrLogUI.MyGotoCommitAction myMyGotoCommitAction;
  ...
  private BzrLogUI.MyTextFieldAction myTextFieldAction;
  private DatesFilterI myDatesFilter;
  private BzrLogSettings mySettings;

  public BzrLogUI(Project project, final Mediator mediator) {
    myProject = project;
    myVcs = BzrVcs.getInstance(project);
    mySettings = BzrLogSettings.getInstance(myProject);
    myMediator = mediator;
    ...
    myFilterStarredAction = new BzrLogShowOnlyStarredCommitsAction(createOnOffForFilterStarred());
  ...
  private void createDetailLoaders() {
    myDetailsLoaderImpl = new Consumer<CommitI>() {
      ...
      public void consume(final CommitI commitI) {
        ...
        final BzrHeavyCommit gitCommit = fullCommitPresentation(commitI);
    ...
    myDetailsLoader = new GenericDetailsLoader<CommitI, BzrHeavyCommit>(myDetailsLoaderImpl, new PairConsumer<CommitI, BzrHeavyCommit>() {
      @Override
      public void consume(CommitI commitI, BzrHeavyCommit commit) {
  ...
  private void selectionChangedForDetails(int[] rows) {
    ...
    if (meaningfulRows == 0) {
      ...
      final BzrHeavyCommit commit = fullCommitPresentation(commitAt);
      if (commit == null) {
  ...
  private BzrHeavyCommit fullCommitPresentation(CommitI commitAt) {
  ...
  private boolean gatherNotLoadedData() {
    ...
    final int[] rows = myJBTable.getSelectedRows();
    final List<BzrHeavyCommit> commits = new ArrayList<BzrHeavyCommit>();
    ...
    for (int i = rows.length - 1; i >= 0; --i) {
      ...
      final BzrHeavyCommit details = fullCommitPresentation(commitI);
      if (details == null) {
    ...
    for (BzrHeavyCommit commit : commits) {
  ...
  private JPanel createMainTable() {
    ...
    myMyChangeListener = new BzrTableScrollChangeListener(myJBTable, myDetailsCache, myTableModel, new Runnable() {
      @Override
      public void run() {
        updateByScroll();
      }
    ...
    myDetailsPanel = new BzrLogDetailsPanel(myProject, myDetailsCache, new Convertor<VirtualFile, CachedRefs>() {
      @Override
  ...
  private void createContextMenu() {
    if (myContextMenu == null) {
      ...
      if (row >= 0) {
        final BzrHeavyCommit commit = getCommitAtRow(row);
        if (commit != null) {
      ...
      myContextMenu = ActionManager.getInstance().createActionPopupMenu(BZR_LOG_TABLE_PLACE, group);
    }
  }
  ...
  private static ActionGroup getCustomActions() {
    return (ActionGroup)ActionManager.getInstance().getAction("Bazaar.LogContextMenu");
  }
  ...
  private ActionToolbar createToolbar() {
    ...
    myDatesFilter = new DatesFilterI() {
      ...
      public void filter(long before, long after, String presetFilterName) {
        final BzrLogSettings.MyDateState state = mySettings.getDateState();
  ...
  private class DataProviderPanel extends JPanel implements TypeSafeDataProvider {
    ...
    private BzrCommitDetailsProvider myCommitDetailsProvider;

    private DataProviderPanel(LayoutManager layout) {
      super(layout);
      myCommitDetailsProvider = new BzrCommitDetailsProvider() {
        @NotNull
        @Override
    ...
    public void calcData(DataKey key, DataSink sink) {
      if (VcsDataKeys.CHANGES.equals(key) || BzrVcs.BZR_COMMIT.equals(key)) {
        final int[] rows = myJBTable.getSelectedRows();
        if (rows.length != 1) return;
        int row = rows[0];
        final BzrHeavyCommit gitCommit = getCommitAtRow(row);
        ...
        else if (BzrVcs.BZR_COMMIT.equals(key)) {
          sink.put(key, gitCommit);
        }
      }
      else if (BzrVcs.SELECTED_COMMITS.equals(key)) {
        sink.put(key, getSelectedCommits());
      }
      else if (BzrVcs.COMMIT_DETAILS_PROVIDER.equals(key)) {
        ...
        final BzrHeavyCommit gitCommit = fullCommitPresentation(commitAt);
        if (gitCommit == null) return;
     ...
     */
    @NotNull
    private List<BzrHeavyCommit> getSelectedCommits() {
      if (myJBTable == null) {
      ...
      for (int row : myJBTable.getSelectedRows()) {
        ...
        final VirtualFile root = commitI.selectRepository(myRootsUnderVcs);
        final BzrHeavyCommit gitCommit = myDetailsCache.convert(root, commitI.getHash());
        if (gitCommit == null) {
          return Collections.emptyList();
        }
  ...
  @Nullable
  private BzrHeavyCommit getCommitAtRow(int row) {
    final CommitI commitAt = myTableModel.getCommitAt(row);
  ...
  private final ColumnInfo<Object, Object> COMMENT = new ColumnInfo<Object, Object>("Comment") {
    ...
    public Object valueOf(Object o) {
      if (o instanceof BzrHeavyCommit) {
        return o;
      }
    ...
    public TableCellRenderer getRenderer(Object o) {
      return o instanceof BzrHeavyCommit ? myDescriptionRenderer : mySimpleRenderer;
    }
  };
  ...
  private class DescriptionRenderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
      ...
      if (value instanceof BzrHeavyCommit) {
        myPanel.removeAll();
        myPanel.setBackground(bg);
        final BzrHeavyCommit commit = (BzrHeavyCommit)value;
    ...
    private List<Trinity<String, Boolean, Color>> getBranchesToDisplay(final BzrHeavyCommit commit, final CommitI commitI) {
    ...
    private class Inner extends HighLightingRenderer {
      ...
      protected void customizeCellRenderer(JTable table, Object value, boolean selected, boolean hasFocus, int row, int column) {
        //setBackground(getLogicBackground(selected, row));
        if (value instanceof BzrHeavyCommit) {
          final BzrHeavyCommit gitCommit = (BzrHeavyCommit)value;
          myIssueLinkRenderer.appendTextWithLinks(gitCommit.getSubject(), SimpleTextAttributes.REGULAR_ATTRIBUTES, myConsumer);
          //super.customizeCellRenderer(table, ((BzrCommit) value).getDescription(), selected, hasFocus, row, column);
        } else {
  ...
  //  Color bkgColor;
  //  final CommitI commitAt = myTableModel.getCommitAt(row);
  //  BzrCommit bzrCommit = null;
  //  if (commitAt != null && (! commitAt.holdsDecoration())) {
  //    bzrCommit = fullCommitPresentation(commitAt);
  //  }
  //
  //  if (isSelected) {
  ...
  //    if (bzrCommit != null) {
  //      if (myDetailsCache.getStashName(commitAt.selectRepository(myRootsUnderVcs), bzrCommit.getShortHash()) != null) {
  //        bkgColor = Colors.stashed;
  //      } else if (bzrCommit.isOnLocal() && bzrCommit.isOnTracked()) {
  //        bkgColor = Colors.commonThisBranch;
  //      } else if (bzrCommit.isOnLocal()) {
  ...
  private void initAuthor() {
    AUTHOR = new ColumnInfo<Object, String>("Author") {
      ...
      public String valueOf(Object o) {
        if (o instanceof BzrHeavyCommit) {
          return ((BzrHeavyCommit)o).getAuthor();
        }
        return "";
      }
  ...
  private final ColumnInfo<Object, String> DATE = new ColumnInfo<Object, String>("Date") {
    ...
    public String valueOf(Object o) {
      if (o instanceof BzrHeavyCommit) {
        return DateFormatUtil.formatPrettyDateTime(((BzrHeavyCommit)o).getDate());
      }
      return "";
    }
  ...
  private void reloadRequest() {
    ...
    final BzrLogSettings.MyDateState dateState = mySettings.getDateState();
    ...
    if (! haveFilters) {
      ...
      myUsersSearchContext.clear();
      myMediator.reload(new RootsHolder(myRootsUnderVcs), startingPoints, null, new BzrLogFilters(), topoOrder);
      ...
      myMediator.reload(new RootsHolder(myRootsUnderVcs), startingPoints, null, new BzrLogFilters(comment, userFilters, dateFilters,
    ...
    public String getUserIfOne() {
      ...
      if (selectedRows != null && selectedRows.length > 0) {
        for (int row : selectedRows) {
          ...
          final BzrHeavyCommit atRow = getCommitAtRow(row);
  ...
  public class MyTreeSettings {
    ...
    private final BzrLogUI.MySelectRootsForTreeAction myRootsForTreeAction;
...
//    private final DumbAwareAction myCurrent;
  ...
  private class MyCheckoutRevisionAction extends DumbAwareAction {
    ...
    public void actionPerformed(AnActionEvent e) {
      ...
      final BzrRepository repository =
        BzrUtil.getRepositoryManager(myProject).getRepositoryForRoot(commitAt.selectRepository(myRootsUnderVcs));
  ...
  private class MyCreateNewTagAction extends DumbAwareAction {
    ...
    public void actionPerformed(AnActionEvent e) {
      ...
      final BzrRepository repository =
        BzrUtil.getRepositoryManager(myProject).getRepositoryForRoot(commitAt.selectRepository(myRootsUnderVcs));
      if (repository == null) return;
      new BzrCreateNewTag(myProject, repository, commitAt.getHash().getString(), myRefresh).execute();
    }
  ...
  public class MyGotoCommitAction extends DumbAwareAction {
    ...
    public void actionPerformed(final AnActionEvent e) {
      ...
      popup[0] = builder.setTitle("Goto")
        ...
        .setDimensionServiceKey(myProject, "Bazaar.Log.Tree.Goto", true)
    ...
    private int getIdx(String reference) {
      ...
      for (VirtualFile root : myRootsUnderVcs) {
        final SHAHash shaHash = BzrChangeUtils.commitExists(myProject, root, reference, null);
        if (shaHash != null) {
      ...
      for (VirtualFile root : myRootsUnderVcs) {
        final List<AbstractHash> abstractHashs = BzrChangeUtils.commitExistsByComment(myProject, root, reference);
