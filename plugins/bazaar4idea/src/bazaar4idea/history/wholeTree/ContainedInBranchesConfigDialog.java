...
public class ContainedInBranchesConfigDialog extends DialogWrapper {
  ...
  protected String getDimensionServiceKey() {
    return "bazaar4idea.history.wholeTree.ContainedInBranchesConfigDialog";
  }
  ...
  private void initUi(Project project, Collection<String> localBranches, Collection<String> remoteBranches, final String currentLocal,
    ...
    final BzrLogSettings bzrLogSettings = BzrLogSettings.getInstance(project);
    setItems(localBranches, bzrLogSettings.getLocalBranchesCopy(), myLocalBranches);
    setItems(remoteBranches, bzrLogSettings.getRemoteBranchesCopy(), myRemoteBranches);

    myHighlight.setSelected(bzrLogSettings.isHighlight());
    myFilter.setSelected(! bzrLogSettings.isHighlight());
  ...
  protected void doOKAction() {
    final BzrLogSettings bzrLogSettings = BzrLogSettings.getInstance(myProject);
    ...
    boolean hightlightChanged = bzrLogSettings.isHighlight() != myHighlight.isSelected();
    bzrLogSettings.setHighlight(myHighlight.isSelected());
    if (bzrLogSettings.setIfChanged(local, remote) || hightlightChanged) {
      myChanged = true;
    }
    super.doOKAction();
  }
