...
import bazaar4idea.history.browser.LowLevelAccessImpl;
...
public class DetailsCache {
  ...
  private final SLRUMap<Pair<VirtualFile, AbstractHash>, BzrHeavyCommit> myCache;
  ...
  private final static Logger LOG = Logger.getInstance("bazaar4idea.history.wholeTree.DetailsCache");
  ...
  public DetailsCache(final Project project,
    ...
    myRefresh = uiRefresh;
    myLock = new Object();
    myCache = new SLRUMap<Pair<VirtualFile, AbstractHash>, BzrHeavyCommit>(ourSize, 150);
    myBranches = new SLRUMap<Pair<VirtualFile, AbstractHash>, List<String>>(20, 20);
  }

  public BzrHeavyCommit convert(final VirtualFile root, final AbstractHash hash) {
    synchronized (myLock) {
  ...
  public void acceptAnswer(final Collection<BzrHeavyCommit> commits, final VirtualFile root) {
    synchronized (myLock) {
  ...
  public void resetAsideCaches() {
    synchronized (myLock) {
      ...
      for (Map.Entry<Pair<VirtualFile, AbstractHash>, BzrHeavyCommit> entry : myCache.entrySet()) {
        final BzrHeavyCommit value = entry.getValue();
  ...
  public void reportRefs(VirtualFile root, CachedRefs refs) {
    synchronized (myLock) {
      ...
      Collection<? extends BzrBranch> local = refs.getLocal();
      for (BzrBranch branch : local) {
        hashes.add(branch.getHash());
      }
      Collection<? extends BzrBranch> remote = refs.getRemote();
      for (BzrBranch branch : remote) {
        hashes.add(branch.getHash());
      }
