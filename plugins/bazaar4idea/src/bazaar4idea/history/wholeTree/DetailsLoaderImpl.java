...
public class DetailsLoaderImpl implements DetailsLoader {
  private final static Logger LOG = Logger.getInstance("#bazaar4idea.history.wholeTree.DetailsLoaderImpl");
  ...
  private class Worker extends Task.Backgroundable {
    ...
    private void loadDetails(Collection<AbstractHash> hashes, SymbolicRefsI refs) throws VcsException {
      ...
      if (! hashes.isEmpty()) {
          final Collection<BzrHeavyCommit> result = myAccess.getCommitDetails(converted, refs);
          if (result != null && (! result.isEmpty())) {
            myDetailsCache.acceptAnswer(result, myAccess.getRoot());
            for (BzrHeavyCommit gitCommit : result) {
              converted.remove(gitCommit.getShortHash().getString());
            }
          }
          if (! converted.isEmpty()) {
            // todo this is bad
            final Collection<BzrHeavyCommit> error = new ArrayList<BzrHeavyCommit>();
    ...
    private BzrHeavyCommit createNotLoadedCommit(AbstractHash shortHash) {
      final String notKnown = "Can not load";
      return new BzrHeavyCommit(NullVirtualFile.INSTANCE, shortHash, SHAHash.emulate(shortHash), notKnown, notKnown, new Date(0), notKnown,
