...
public class LoadController implements Loader {
  ...
  private final BzrCommitsSequentially myBzrCommitsSequentially;
  private LoadAlgorithm myPreviousAlgorithm;
  private NewBzrUsersComponent myUsersComponent;
  private static final Logger LOG = Logger.getInstance("#bazaar4idea.history.wholeTree.LoadController");
  private final Map<VirtualFile, Long> mySortOrder;

  public LoadController(final Project project, final Mediator mediator, final DetailsCache detailsCache, final BzrCommitsSequentially bzrCommitsSequentially) {
    myProject = project;
    myMediator = mediator;
    myDetailsCache = detailsCache;
    myBzrCommitsSequentially = bzrCommitsSequentially;
    myUsersIndex = new UsersIndex();
    myUsersComponent = NewBzrUsersComponent.getInstance(myProject);
  ...
  public void loadSkeleton(final Ticket ticket,
                           ...
                           final BzrLogFilters bzrLogFilters,
    ...
    for (VirtualFile root : roots) {
      ...
      if (! bzrLogFilters.isUseOnlyHashes()) {
        final boolean haveStructureFilter = bzrLogFilters.haveStructureFilter();
        // check if no files under root are selected
        if (haveStructureFilter && ! bzrLogFilters.haveStructuresForRoot(root)) {
          ++ i;
          continue;
        }
        bzrLogFilters.callConsumer(new Consumer<List<ChangesFilter.Filter>>() {
          @Override
          public void consume(final List<ChangesFilter.Filter> filters) {
            final LoaderAndRefresherImpl loaderAndRefresher =
              new LoaderAndRefresherImpl(ticket, filters, myMediator, startingPoints, myDetailsCache, myProject, rootHolder, myUsersIndex,
                                         loadGrowthController.getId(), haveStructureFilter, topoOrder, bzrLogFilters.haveDisordering());
            list.add(loaderAndRefresher);
          }
        }, true, root);
      }

      shortLoaders.add(new ByRootLoader(myProject, rootHolder, myMediator, myDetailsCache, ticket, myUsersIndex, bzrLogFilters, startingPoints));
      ++ i;
    }
  ...
  private class RepositoriesSorter extends TaskDescriptor {
    ...
    private RepositoriesSorter(final List<LoaderAndRefresher<CommitHashPlusParents>> simpleLoaders, final List<ByRootLoader> shortLoaders) {
      ...
      myPreviousAlgorithm = new LoadAlgorithm(myProject, mySimpleLoaders, myShortLoaders);
    }
    ...
    public void run(ContinuationContext context) {
      ...
      for (ByRootLoader shortLoader : myShortLoaders) {
        ...
        try {
          long ts = BzrHistoryUtils.getHeadTs(myProject, new FilePathImpl(root));
