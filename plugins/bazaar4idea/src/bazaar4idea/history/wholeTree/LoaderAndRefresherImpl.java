...
public class LoaderAndRefresherImpl implements LoaderAndRefresher<CommitHashPlusParents> {
  ...
  private void loadFull(final int count, final long continuation) {
    try {
      ...
      myLowLevelAccess.loadCommits(myStartingPoints, Collections.<String>emptyList(), filters, new AsynchConsumer<BzrHeavyCommit>() {
        @Override
        public void consume(BzrHeavyCommit gitCommit) {
          if (gitCommit.getParentsHashes().size() <= 1) {
            myDetailsCache.acceptAnswer(Collections.singleton(gitCommit), myRootHolder.getRoot());
          }
          myRepeatingLoadConsumer.consume(BzrCommitToCommitConvertor.getInstance().convert(gitCommit));
        }
  ...
  private void appendCommits(List<CommitI> result, List<List<AbstractHash>> parents, List<BzrHeavyCommit> commits) {
