 ...
 */
interface BzrHttpRemoteCommand {

  String getUrl();
  void setUrl(String url);
  void run() throws GitAPIException, URISyntaxException, TransportException;
  void cleanup();
  BzrHttpCredentialsProvider getCredentialsProvider();
  String getLogString();
  String getCommandString();

  class Fetch implements BzrHttpRemoteCommand {

    private final Git myBzr;
    private final BzrHttpCredentialsProvider myCredentialsProvider;
    private String myUrl;
    private final List<RefSpec> myRefSpecs;

    Fetch(@NotNull Git bzr, @NotNull BzrHttpCredentialsProvider credentialsProvider, @NotNull String url, @NotNull List<RefSpec> refSpecs) {
      myBzr = bzr;
    ...
    public void run() throws GitAPIException {
      FetchCommand fetchCommand = myBzr.fetch();
    ...
    @Override
    public BzrHttpCredentialsProvider getCredentialsProvider() {
  ...
  class Clone implements BzrHttpRemoteCommand {

    private final File myTargetDirectory;
    private final BzrHttpCredentialsProvider myCredentialsProvider;
    private String myUrl;
    @Nullable private Git myBzr;

    Clone(@NotNull File targetDirectory, @NotNull BzrHttpCredentialsProvider credentialsProvider, String url) {
  ...
  class Push implements BzrHttpRemoteCommand {

    private final Git myGit;
    private final BzrHttpCredentialsProvider myCredentialsProvider;
    private BzrSimplePushResult myPushResult;
    private String myRemoteName;
    private String myUrl;
    private final List<RefSpec> myPushSpecs;

    Push(@NotNull Git git, @NotNull BzrHttpCredentialsProvider credentialsProvider, @NotNull String remoteName, @NotNull String url, @NotNull List<RefSpec> pushSpecs) {
      myGit = git;
      myCredentialsProvider = credentialsProvider;
    ...
    @Override
    public String getLogString() {
      return String.format("git push %s (%s) %s", myRemoteName, myUrl, BzrHttpRemoteCommand.Fetch.getRefspecsAsString(myPushSpecs));
    }

    @Override
    public String getCommandString() {
      return String.format("git push %s %s", myRemoteName, BzrHttpRemoteCommand.Fetch.getRefspecsAsString(myPushSpecs));
    }
    ...
    @Nullable
    BzrSimplePushResult getResult() {
      return myPushResult;
    }

    @NotNull
    private static BzrSimplePushResult analyzeResults(@NotNull Iterable<PushResult> results) {
      ...
      if (!rejectedBranches.isEmpty()) {
        ...
        return BzrSimplePushResult.error(errorReport.toString());
      }
    }
  ...
  class LsRemote implements BzrHttpRemoteCommand {
    ...
    public LsRemote(@NotNull Git bzr, @NotNull BzrHttpCredentialsProvider credentialsProvider, @NotNull String url) {
      myBzr = bzr;
      myCredentialsProvider = credentialsProvider;
      myUrl = url;
    }
    ...
    private Collection<Ref> call() throws TransportException, InvalidRemoteException {
      try {
        Transport transport = Transport.open(myBzr.getRepository(), myUrl);
