...
/**
 *
 * The class is highly customizable, since the procedure of resolving conflicts is very common in Bazaar operations.
 * @author Kirill Likhodedov
 */
public class BzrConflictResolver {

  private static final Logger LOG = Logger.getInstance(BzrConflictResolver.class);
  ...
  public BzrConflictResolver(@NotNull Project project,
                             @NotNull Bzr bzr,
   ...
   * </p>
   * <p>
   *   If a Bazaar error happens during seeking for unmerged files or in other cases,
    ...
    UIUtil.invokeAndWaitIfNeeded(new Runnable() {
      @Override public void run() {
        final MergeProvider mergeProvider = myParams.reverse ?
                                            new BzrMergeProvider(myProject, true) : new BzrMergeProvider(myProject, false);
  ...
  private void notifyException(VcsException e) {
    ...
    BzrVcs.IMPORTANT_ERROR_NOTIFICATION.createNotification(myParams.myErrorNotificationTitle,
  ...
  /**
   * @return unmerged files in the given Bazaar roots, all in a single collection.
  ...
  /**
   * @return unmerged files in the given Bazaar root.
  ...
  private List<VirtualFile> unmergedFiles(VirtualFile root) throws VcsException {
    ...
    BzrCommandResult result = myBzr.getUnmergedFiles(repository);
    if (!result.success()) {
    ...
    for (StringScanner s = new StringScanner(output); s.hasMoreData();) {
      ...
      String path = rootPath + "/" + BzrUtil.unescapePath(relative);
    ...
    if (unmerged.size() == 0) {
      ...
      rc.addAll(unmerged);
      Collections.sort(rc, BzrUtil.VIRTUAL_FILE_COMPARATOR);
      return rc;
    }
  }
