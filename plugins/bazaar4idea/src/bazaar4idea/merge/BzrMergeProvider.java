...
/**
 * Merge-changes provider for Bazaar, used by IDEA internal 3-way merge tool
 */
public class BzrMergeProvider implements MergeProvider2 {
  ...
  private static final Logger LOG = Logger.getInstance(BzrMergeProvider.class);
  ...
  private BzrMergeProvider(@NotNull Project project, @NotNull Set<VirtualFile> reverseRoots) {
    myProject = project;
    myReverseRoots = reverseRoots;
  }

  public BzrMergeProvider(@NotNull Project project, boolean reverse) {
  ...
  public static MergeProvider detect(@NotNull Project project) {
    return new BzrMergeProvider(project, findReverseRoots(project, ReverseRequest.DETECT));
  }
  ...
  private static Set<VirtualFile> findReverseRoots(@NotNull Project project, @NotNull ReverseRequest reverseOrDetect) {
    ...
    for (BzrRepository repository : BzrUtil.getRepositoryManager(project).getRepositories()) {
      boolean reverse;
      if (reverseOrDetect == ReverseRequest.DETECT) {
        reverse = repository.getState().equals(BzrRepository.State.REBASING);
      }
      else {
  ...
  public MergeData loadRevisions(final VirtualFile file) throws VcsException {
    ...
    final VirtualFile root = BzrUtil.getBzrRoot(file);
    final FilePath path = VcsUtil.getFilePath(file.getPath());

    VcsRunnable runnable = new VcsRunnable() {
      @SuppressWarnings({"ConstantConditions"})
      public void run() throws VcsException {
        BzrFileRevision original = new BzrFileRevision(myProject, path, new BzrRevisionNumber(":" + ORIGINAL_REVISION_NUM));
        BzrFileRevision current = new BzrFileRevision(myProject, path, new BzrRevisionNumber(":" + yoursRevision(root)));
        BzrFileRevision last = new BzrFileRevision(myProject, path, new BzrRevisionNumber(":" + theirsRevision(root)));
        try {
          try {
    ...
    VcsUtil.runVcsProcessWithProgress(runnable, BzrBundle.message("merge.load.files"), false, myProject);
    return mergeData;
  }
  ...
  private VcsRevisionNumber findLastRevisionNumber(@NotNull VirtualFile root) {
    if (myReverseRoots.contains(root)) {
      ...
      try {
        return BzrRevisionNumber.resolve(myProject, root, "MERGE_HEAD");
      }
      catch (VcsException e) {
        LOG.info("Couldn't resolve the MERGE_HEAD in " + root, e); // this may be not a bug, just cherry-pick
        try {
          return BzrRevisionNumber.resolve(myProject, root, "CHERRY_PICK_HEAD");
        }
  ...
  @Nullable
  private BzrRevisionNumber resolveHead(@NotNull VirtualFile root) {
    try {
      return BzrRevisionNumber.resolve(myProject, root, "HEAD");
    }
    catch (VcsException e) {
  ...
  private static byte[] loadRevisionCatchingErrors(@NotNull BzrFileRevision revision) throws VcsException, IOException {
    try {
  ...
  public void conflictResolvedForFile(VirtualFile file) {
    if (file == null) return;
    try {
      BzrFileUtils.addFiles(myProject, BzrUtil.getBzrRoot(file), file);
    }
    catch (VcsException e) {
  ...
  private class MyMergeSession implements MergeSession {
    ...
    MyMergeSession(List<VirtualFile> filesToMerge) {
      ...
      try {
        for (Map.Entry<VirtualFile, List<VirtualFile>> e : BzrUtil.sortFilesByBzrRoot(filesToMerge).entrySet()) {
    ...
    public void conflictResolvedForFile(VirtualFile file, Resolution resolution) {
      ...
      try {
        ...
        switch (status) {
          case MODIFIED:
            BzrFileUtils.addFiles(myProject, c.myRoot, file);
            break;
          case DELETED:
            BzrFileUtils.deleteFiles(myProject, c.myRoot, file);
            break;
          default:
    ...
    class StatusColumn extends ColumnInfo<VirtualFile, String> {
      ...
      public StatusColumn(boolean isTheirs) {
        super(isTheirs ? BzrBundle.message("merge.tool.column.theirs.status") : BzrBundle.message("merge.tool.column.yours.status"));
        myIsTheirs = isTheirs;
      }
      ...
      public String valueOf(VirtualFile file) {
        ...
        switch (s) {
          case MODIFIED:
            return BzrBundle.message("merge.tool.column.status.modified");
          case DELETED:
            return BzrBundle.message("merge.tool.column.status.deleted");
          default:
