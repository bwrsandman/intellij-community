...
import bazaar4idea.actions.BzrRepositoryAction;
 ...
 * Utilities for merge
 */
public class BzrMergeUtil {
  /**
   * The item representing default strategy
   */
  public static final String DEFAULT_STRATEGY = BzrBundle.getString("merge.default.strategy");

  /**
   * A private constructor for utility class
   */
  private BzrMergeUtil() {
  }
   ...
   */
  public static void showUpdates(BzrRepositoryAction action,
                                 final Project project,
                                 final List<VcsException> exceptions,
                                 final VirtualFile root,
                                 final BzrRevisionNumber currentRev,
  ...
  private static File getMergeHead(@NotNull VirtualFile root) {
    ...
    File f = new File(gitDir, BzrRepositoryFiles.MERGE_HEAD);
    if (f.exists()) {
      return f;
    }
    return null;
