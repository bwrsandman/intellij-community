...
public class MergeChangeCollector {
  ...
  private final BzrRevisionNumber myStart; // Revision number before update (used for diff)

  public MergeChangeCollector(final Project project, final VirtualFile root, final BzrRevisionNumber start) {
    myStart = start;
    myProject = project;
    myRoot = root;
  }
  ...
  public @NotNull Set<String> getUnmergedPaths() throws VcsException {
    ...
    for (StringScanner s = new StringScanner(result); s.hasMoreData();) {
      ...
      String path = root + "/" + BzrUtil.unescapePath(relative);
      paths.add(path);
    }
    return paths;
  }
  ...
  public String getRevisionsForDiff() throws VcsException {
    String root = myRoot.getPath();
    BzrRevisionNumber currentHead = BzrRevisionNumber.resolve(myProject, myRoot, "HEAD");
    if (currentHead.equals(myStart)) {
      ...
      File mergeHeadsFile = new File(root, BzrRepositoryFiles.BAZAAR_MERGE_HEAD);
      try {
        if (mergeHeadsFile.exists()) {
          String mergeHeads = new String(FileUtil.loadFileText(mergeHeadsFile, BzrUtil.UTF8_ENCODING));
  ...
  private static void addAll(final UpdatedFiles updates, String group_id, Set<String> paths) {
    ...
    final VcsKey vcsKey = BzrVcs.getKey();
