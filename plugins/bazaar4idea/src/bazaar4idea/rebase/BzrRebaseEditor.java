 ...
 */
public class BzrRebaseEditor extends DialogWrapper {
  /**
   ...
   * The git root
   */
  private final VirtualFile myBzrRoot;
  /**
   ...
   * @param bzrRoot the Bazaar root
   * @param file    the file to edit
   * @throws IOException if file could not be loaded
   */
  protected BzrRebaseEditor(final Project project, final VirtualFile bzrRoot, String file) throws IOException {
    super(project, true);
    myProject = project;
    myBzrRoot = bzrRoot;
    setTitle(BzrBundle.getString("rebase.editor.title"));
    setOKButtonText(BzrBundle.getString("rebase.editor.button"));
    ...
    for (Object option : BzrRebaseEntry.Action.values()) {
    ...
    myViewButton.addActionListener(new ActionListener() {
      public void actionPerformed(final ActionEvent e) {
        ...
        BzrRebaseEntry entry = myTableModel.myEntries.get(row);
        BzrUtil.showSubmittedFiles(project, entry.getCommit(), bzrRoot, false, false);
      }
    });
  ...
  private void validateFields() {
    final List<BzrRebaseEntry> entries = myTableModel.myEntries;
    if (entries.size() == 0) {
      setErrorText(BzrBundle.getString("rebase.editor.invalid.entryset"));
      setOKActionEnabled(false);
      return;
    }
    int i = 0;
    while (i < entries.size() && entries.get(i).getAction() == BzrRebaseEntry.Action.skip) {
      i++;
    }
    if (i < entries.size() && entries.get(i).getAction() == BzrRebaseEntry.Action.squash) {
      setErrorText(BzrBundle.getString("rebase.editor.invalid.squash"));
      setOKActionEnabled(false);
      return;
    }
    setErrorText(null);
  ...
  protected String getHelpId() {
    return "reference.VersionControl.Bazaar.RebaseCommits";
  }
  ...
  private class MyTableModel extends AbstractTableModel {
     ...
     * The entries
     */
    final List<BzrRebaseEntry> myEntries = new ArrayList<BzrRebaseEntry>();
    ...
    public String getColumnName(final int column) {
      switch (column) {
        case ACTION:
          return BzrBundle.getString("rebase.editor.action.column");
        case COMMIT:
          return BzrBundle.getString("rebase.editor.commit.column");
        case SUBJECT:
          return BzrBundle.getString("rebase.editor.comment.column");
        default:
    ...
    public Object getValueAt(final int rowIndex, final int columnIndex) {
      BzrRebaseEntry e = myEntries.get(rowIndex);
      switch (columnIndex) {
        case ACTION:
          return new ListWithSelection<BzrRebaseEntry.Action>(Arrays.asList(BzrRebaseEntry.Action.values()), e.getAction());
        case COMMIT:
          return e.getCommit();
        case SUBJECT:
          return e.getSubject();
    ...
    private void setRowAction(Object aValue, int rowIndex, int columnIndex) {
      BzrRebaseEntry e = myEntries.get(rowIndex);
      e.setAction((BzrRebaseEntry.Action)aValue);
    ...
    public void load(final String file) throws IOException {
      String encoding = BzrConfigUtil.getLogEncoding(myProject, myBzrRoot);
      ...
      while (s.hasMoreData()) {
        ...
        myEntries.add(new BzrRebaseEntry(hash, comment));
      }
    }
    ...
    public void save(final String file) throws IOException {
      ...
      try {
        for (BzrRebaseEntry e : myEntries) {
          if (e.getAction() != BzrRebaseEntry.Action.skip) {
