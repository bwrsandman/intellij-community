 ...
 */
public class BzrRebaseEditorService {
  /**
   ...
   * The handlers to use
   */
  private final Map<Integer, BzrRebaseEditorHandler> myHandlers = new THashMap<Integer, BzrRebaseEditorHandler>();
  /**
   ...
   */
  @NonNls private static final String BZR_REBASE_EDITOR_PREFIX = "bzr-rebase-editor-";

  /**
   * @return an instance of the server
   */
  @NotNull
  public static BzrRebaseEditorService getInstance() {
    final BzrRebaseEditorService service = ServiceManager.getService(BzrRebaseEditorService.class);
    if (service == null) {
      throw new IllegalStateException("The service " + BzrRebaseEditorService.class.getName() + " cannot be located");
    }
    return service;
  }
  ...
  private void addInternalHandler() {
    ...
    if (!xmlRpcServer.hasHandler(BzrRebaseEditorMain.HANDLER_NAME)) {
      xmlRpcServer.addHandler(BzrRebaseEditorMain.HANDLER_NAME, new InternalHandler());
    }
  }
  ...
  public synchronized String getEditorCommand() {
    synchronized (myScriptLock) {
      if (myEditorCommand == null) {
        ScriptGenerator generator = new ScriptGenerator(BZR_REBASE_EDITOR_PREFIX, BzrRebaseEditorMain.class);
   ...
   */
  public int registerHandler(BzrRebaseEditorHandler handler) {
    addInternalHandler();
    Integer rc = null;
   ...
   */
  @NotNull
  BzrRebaseEditorHandler getHandler(final int handlerNo) {
    synchronized (myHandlersLock) {
      BzrRebaseEditorHandler h = myHandlers.get(handlerNo);
      if (h == null) {
   ...
   */
  public void configureHandler(BzrLineHandler h, int editorNo) {
    h.setEnvironment(BzrCommand.BZR_EDITOR_ENV, getEditorCommand());
    h.setEnvironment(BzrRebaseEditorMain.IDEA_REBASE_HANDER_NO, Integer.toString(editorNo));
  }
  ...
  public class InternalHandler {
    ...
    public int editCommits(int handlerNo, String path) {
      BzrRebaseEditorHandler editor = getHandler(handlerNo);
      BzrHandler handler = editor.getHandler();
