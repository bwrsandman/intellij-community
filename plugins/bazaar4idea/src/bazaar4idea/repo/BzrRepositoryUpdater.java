...
/**
 * Listens to .git service files changes and updates {@link BzrRepository} when needed.
 * @author Kirill Likhodedov
 */
final class BzrRepositoryUpdater implements Disposable, BulkFileListener {
  @NotNull private final BzrRepositoryFiles myRepositoryFiles;
  ...
  BzrRepositoryUpdater(@NotNull BzrRepository repository) {
    VirtualFile bzrDir = repository.getBzrDir();
    myWatchRequest = LocalFileSystem.getInstance().addRootToWatch(bzrDir.getPath(), true);

    myRepositoryFiles = BzrRepositoryFiles.getInstance(bzrDir);
    RepositoryUtil.visitVcsDirVfs(bzrDir, BzrRepositoryFiles.getSubDirRelativePaths());
  ...
  public void after(@NotNull List<? extends VFileEvent> events) {
    ...
    for (VFileEvent event : events) {
      ...
      filePath = BzrFileUtils.stripFileProtocolPrefix(filePath);
