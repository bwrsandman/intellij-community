...
/**
 * <p>The provider of external application scripts called by Bazaar when a remote operation needs communication with the user.</p>
 * <p>
 *   Usage:
 *   <ol>
 *     <li>Get the script from {@link #getScriptPath()}.</li>
 *     <li>Set up proper environment variable
 *         (e.g. {@code BAZAAR_SSH} for SSH connections, or {@code BAZAAR_ASKPASS} for HTTP) pointing to the script.</li>
 *     <li>{@link #registerHandler(Object) Register} the handler of Bazaar requests.</li>
 *     <li>Call Bazaar operation.</li>
...
public abstract class BzrXmlRpcHandlerService<T> {
   ...
   * @param aClass      Main class of the external application invoked by Bazaar,
   *                    which is able to handle its requests and pass to the main IDEA instance.
   */
  protected BzrXmlRpcHandlerService(@NotNull String prefix, @NotNull String handlerName, @NotNull Class<? extends BazaarExternalApp> aClass) {
