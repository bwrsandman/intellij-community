 ...
 */
public abstract class BzrChangesSaver {

  private static final Logger LOG = Logger.getInstance(BzrChangesSaver.class);

  @NotNull protected final Project myProject;
  @NotNull protected final BzrPlatformFacade myPlatformFacade;
  ...
  protected BzrConflictResolver.Params myParams;
  ...
  /**
   * Returns an instance of the proper BzrChangesSaver depending on the chosen save changes policy.
   * @return {@link BzrStashChangesSaver}, {@link BzrShelveChangesSaver} or {@link BzrDumbChangesSaver}
   */
  public static BzrChangesSaver getSaver(@NotNull Project project, @NotNull BzrPlatformFacade platformFacade, @NotNull Bzr bzr,
    ...
    if (settings == null) {
      return getDefaultSaver(project, platformFacade, bzr, progressIndicator, stashMessage);
    }
    switch (settings.updateChangesPolicy()) {
      case STASH: return new BzrStashChangesSaver(project, platformFacade, bzr, progressIndicator, stashMessage);
      case SHELVE: return new BzrShelveChangesSaver(project, platformFacade, bzr, progressIndicator, stashMessage);
    }
    return getDefaultSaver(project, platformFacade, bzr, progressIndicator, stashMessage);
  }

  // In the case of illegal value in the settings or impossibility to get the settings.
  private static BzrChangesSaver getDefaultSaver(@NotNull Project project, @NotNull BzrPlatformFacade platformFacade, @NotNull Bzr bzr,
                                                 @NotNull ProgressIndicator progressIndicator, @NotNull String stashMessage) {
    return new BzrStashChangesSaver(project, platformFacade, bzr, progressIndicator, stashMessage);
  }

  protected BzrChangesSaver(@NotNull Project project,
  ...
  public void notifyLocalChangesAreNotRestored() {
    if (wereChangesSaved()) {
      ...
      BzrVcs.IMPORTANT_ERROR_NOTIFICATION.createNotification("Local changes were not restored",
  ...
  public void setConflictResolverParams(BzrConflictResolver.Params params) {
    myParams = params;
  }
