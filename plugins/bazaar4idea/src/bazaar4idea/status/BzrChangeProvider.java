...
/**
 * Bazaar repository change provider
 */
public class BzrChangeProvider implements ChangeProvider {

  private static final Logger PROFILE_LOG = Logger.getInstance("#BzrStatus");
  ...
  public void getChanges(final VcsDirtyScope dirtyScope,
    ...
    Collection<VirtualFile> roots = BzrUtil.bzrRootsForPaths(affected);
    ...
    try {
      ...
      for (VirtualFile root : roots) {
        ...
        BzrChangesCollector collector = BzrNewChangesCollector.collect(myProject, myBzr, myChangeListManager, myVcsManager, vcs, dirtyScope, root);
        ...
        for (Change file : changes) {
          ...
          builder.processChange(file, BzrVcs.getKey());
        }
      ...
      PROFILE_LOG.info(e);
      // most probably the error happened because Bazaar is not configured
      vcs.getExecutableValidator().showNotificationOrThrow(e);
    }
  }

  public static void appendNestedVcsRootsToDirt(final VcsDirtyScope dirtyScope, BzrVcs vcs, final ProjectLevelVcsManager vcsManager) {
    ...
    final Set<VirtualFile> rootsUnderBzr = new HashSet<VirtualFile>(Arrays.asList(vcsManager.getRootsUnderVcs(vcs)));
    final Set<VirtualFile> inputColl = new HashSet<VirtualFile>(rootsUnderBzr);
  ...
  /**
   * Common debug logging method for all Bazaar status related operations.
  ...
  private static class MyNonChangedHolder {
    ...
    public void feedBuilder(final ChangelistBuilder builder) throws VcsException {
      final VcsKey gitKey = BzrVcs.getKey();
      ...
      for (FilePath filePath : myDirty) {
        ...
        if (vf != null) {
          if ((myAddGate.getStatus(vf) == null) && myFileDocumentManager.isFileModified(vf)) {
            ...
            if (root != null) {
              final BzrRevisionNumber beforeRevisionNumber = BzrChangeUtils.resolveReference(myProject, root, "HEAD");
              builder.processChange(new Change(BzrContentRevision.createRevision(vf, beforeRevisionNumber, myProject),
                                               BzrContentRevision.createRevision(vf, null, myProject), FileStatus.MODIFIED), gitKey);
            }
          }
        }
      }
