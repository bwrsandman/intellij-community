 ...
 */
public class BzrSkippedCommits extends PanelWithActionsAndCloseButton {
  /**
   * The current project
   */
   ...
   */
  public BzrSkippedCommits(@NotNull ContentManager contentManager,
                           Project project,
                           SortedMap<VirtualFile, List<BzrRebaseUtils.CommitInfo>> skippedCommits) {
    super(contentManager, null);
    myProject = project;
    DefaultMutableTreeNode treeRoot = new DefaultMutableTreeNode("ROOT", true);
    for (Map.Entry<VirtualFile, List<BzrRebaseUtils.CommitInfo>> e : skippedCommits.entrySet()) {
      DefaultMutableTreeNode vcsRoot = new DefaultMutableTreeNode(new VcsRoot(e.getKey()));
      int missed = 0;
      for (BzrRebaseUtils.CommitInfo c : e.getValue()) {
        if (c != null) {
  ...
  public Object getData(String dataId) {
    ...
    if (o instanceof Commit) {
      ...
      if (VcsDataKeys.VCS_FILE_REVISION.is(dataId)) {
        return new BzrFileRevision(myProject, VcsUtil.getFilePath(c.root.getPath()), c.commitInfo.revision);
      }
    }
   ...
   */
  public static void showSkipped(final Project project, final SortedMap<VirtualFile, List<BzrRebaseUtils.CommitInfo>> skippedCommits) {
    UIUtil.invokeLaterIfNeeded(new Runnable() {
      ...
      public void run() {
        ...
        BzrSkippedCommits skipped = new BzrSkippedCommits(contentManager, project, skippedCommits);
  ...
  private static class Commit {
    final VirtualFile root;
    final BzrRebaseUtils.CommitInfo commitInfo;

    public Commit(VirtualFile root, BzrRebaseUtils.CommitInfo commitInfo) {
      this.root = root;
