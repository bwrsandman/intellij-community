...
public class BzrCherryPickStepdefs {
  ...
  private static void commitInFuture(final int times) {
    myVcsHelper.registerHandler(new MockVcsHelper.CommitHandler() {
      ...
      public boolean commit(String commitMessage) {
        ...
        myCommitRequests++;
        BzrExecutor.bzr(String.format("commit -am '%s'", commitMessage));
        return true;
      }
    });
  }
  ...
  public void the_last_commit_is_hash(String hash) throws Throwable {
    assertEquals("The last commit hash doesn't match", virtualCommits.replaceVirtualHashes(hash), BzrExecutor.bzr("log -1 --pretty=%h"));
  }
  ...
  public void working_tree_is_dirty() throws Throwable {
    assertFalse("Working tree is unexpectedly clean", BzrExecutor.bzr("diff").trim().isEmpty() && BzrExecutor.bzr("diff --cached").trim().isEmpty());
  }
  ...
  private static void cherryPick(List<String> virtualHashes) {
    ...
    new BzrCherryPicker(myProject, ourMyBzr, myPlatformFacade, mySettings.isAutoCommitOnCherryPick())
  ...
  private static VcsFullCommitDetails mockCommit(String hash, String message) {
    ...
    return ServiceManager.getService(myProject, VcsLogObjectsFactory.class).createFullDetails(
      ...
      "John Smith", "john@mail.com", 0, changes, BzrContentRevisionFactory.getInstance(myProject));
  }
