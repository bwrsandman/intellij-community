...
import bazaar4idea.remote.BzrHttpAuthTestService;
import bazaar4idea.repo.BzrRepository;
import bazaar4idea.test.BzrTestUtil;
...
import bazaar4idea.commands.BzrHttpAuthService;
...
public class BzrCucumberWorld {
  ...
  public static BzrPlatformFacade myPlatformFacade;
  public static Bzr ourMyBzr;
  public static BzrRepository myRepository;
  public static BzrVcsSettings mySettings;
  public static ChangeListManagerImpl myChangeListManager;
  public static BzrVcs myVcs;

  public static MockVcsHelper myVcsHelper;
  public static TestNotificator myNotificator;

  public static BzrHttpAuthTestService myHttpAuthService; // only with @remote tag

  public static BzrTestVirtualCommitsHolder virtualCommits;
  ...
  public void setUp() throws Throwable {
    ...
    ourMyBzr = ServiceManager.getService(myProject, Bzr.class);
    mySettings = myPlatformFacade.getSettings(myProject);
    mySettings.getAppSettings().setPathToBzr(BzrExecutor.Bzr_EXECUTABLE);
    ...
    virtualCommits = new BzrTestVirtualCommitsHolder();
    myAsyncTasks = new ArrayList<Future>();

    cd(myProjectRoot);
    myRepository = BzrTestUtil.createRepository(myProject, myProjectRoot);

    ProjectLevelVcsManagerImpl vcsManager = (ProjectLevelVcsManagerImpl)ProjectLevelVcsManager.getInstance(myProject);
    AbstractVcs vcs = vcsManager.findVcsByName("Bazaar");
    Assert.assertEquals(1, vcsManager.getRootsUnderVcs(vcs).length);

    assumeSupportedBzrVersion();
  }

  private static void assumeSupportedBzrVersion() {
  ...
  public void setUpRemoteOperations() {
    BzrTestUtil.setDefaultBuiltInServerPort();
    myHttpAuthService = (BzrHttpAuthTestService)ServiceManager.getService(BzrHttpAuthService.class);
  }
  ...
  public void setUpStandardMultipleRootsConfig() {
    ...
    BzrTestUtil.createRepository(myProject, community);
  }
  ...
  private static void nullifyStaticFields() throws IllegalAccessException {
    for (Field field : BzrCucumberWorld.class.getDeclaredFields()) {
