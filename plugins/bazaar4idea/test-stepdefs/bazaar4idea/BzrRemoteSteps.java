...
public class BzrRemoteSteps {
  ...
  public void I_clone_the_repository(final String url) {
    ...
    executeOnPooledThread(new Runnable() {
      @Override
      public void run() {
        String projectName = url.substring(url.lastIndexOf('/') + 1).replace(".bzr", "");
        BzrCheckoutProvider.doClone(myProject, new EmptyProgressIndicator(), ourMyBzr, projectName, myTestRoot, url);
  ...
  private static class TestAuthenticator implements BzrHttpAuthenticator {
