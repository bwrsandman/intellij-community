 ...
 */
public class BzrMockVcsManager extends ProjectLevelVcsManager {
  ...
  BzrMockVcsManager(Project project, DvcsPlatformFacade facade) {
    myProject = project
    myPlatformFacade = facade
    myVcs = facade.getVcs(project)
  }

  // TODO remove and use getting all roots from BzrRepositoryManager.
  ...
  List<VcsDirectoryMapping> getDirectoryMappings() {
    List<VcsDirectoryMapping> roots = myRoots.collect { new VcsDirectoryMapping(it, "Bazaar") }
    if (myProjectRootMapping) {
      roots << new VcsDirectoryMapping("", "Bazaar")
    }
    roots
  }
