...
/**
 * Representation of a Bazaar repository for tests purposes.
 ...
 */
@Deprecated
public class BzrTestRepository {

  private File myRootDir;
  private BzrTestRunEnv myRunEnv;

  @NotNull
  public static BzrTestRepository init(@NotNull File dir) throws IOException {
    BzrTestRepository repo = new BzrTestRepository(dir);
    repo.init();
    return repo;
  }

  @NotNull
  public static BzrTestRepository clone(@NotNull BzrTestRepository sourceRepo, @NotNull File targetDir) throws IOException {
    sourceRepo.run("clone", sourceRepo.getRootDir().getPath(), targetDir.getPath());
    return new BzrTestRepository(targetDir);
  }

  /**
   * Create new BzrTestRepository related to the given directory - the Bazaar repository root.
   * @param rootDir
   */
  public BzrTestRepository(@NotNull File rootDir) {
    myRootDir = rootDir;
    myRunEnv = new BzrTestRunEnv(myRootDir);
  }
  ...
  /**
   * Configures name and email for this bzr repository.
   */
