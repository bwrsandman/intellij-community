...
public class BzrTestUtil {
   ...
   */
  public static Map<String, VirtualFile> createFileStructure(Project project, BzrTestRepository repo, String... paths) {
  ...
  public static void initRepo(@NotNull String repoRoot) {
    cd(repoRoot);
    BzrExecutor.bzr("init");
    setupUsername();
    touch("initial.txt");
    BzrExecutor.bzr("add initial.txt");
    BzrExecutor.bzr("commit -m initial");
  }

  public static void setupUsername() {
    BzrExecutor.bzr("config user.name " + USER_NAME);
    BzrExecutor.bzr("config user.email " + USER_EMAIL);
  }

  /**
   * Creates a Bazaar repository in the given root directory;
   * registers it in the Settings;
   * return the {@link bazaar4idea.repo.BzrRepository} object for this newly created repository.
   */
  @NotNull
  public static BzrRepository createRepository(@NotNull Project project, @NotNull String root) {
    initRepo(root);
    ProjectLevelVcsManagerImpl vcsManager = (ProjectLevelVcsManagerImpl)ProjectLevelVcsManager.getInstance(project);
    vcsManager.setDirectoryMapping(root, BzrVcs.NAME);
    VirtualFile file = LocalFileSystem.getInstance().findFileByIoFile(new File(root));
    BzrRepository repository = BzrUtil.getRepositoryManager(project).getRepositoryForRoot(file);
