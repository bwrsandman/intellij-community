...
import static bazaar4idea.test.BzrScenarios.*
 ...
 */
@Ignore
class BzrBranchWorkerTest extends BzrLightTest {

  private BzrRepository myUltimate
  private BzrRepository myCommunity
  private BzrRepository myContrib

  private List<BzrRepository> myRepositories
  ...
  boolean newBzrVersion() {
    return !BzrVersionSpecialty.OLD_STYLE_OF_UNTRACKED_AND_LOCAL_CHANGES_WOULD_BE_OVERWRITTEN.existsIn(BzrVersion.parse(bzr("version")));
  }
  ...
  Collection<String> prepareLocalChangesOverwrittenBy(BzrRepository repository, int numFiles = 1) {
    def localChanges = []
    ...
    // Probably we should create BzrTestChangeListManager that would fairly call bzr status and analyze the output.
    // Maybe we could reuse BzrChangeProvider or at least BzrNewChangesCollector.
  ...
  public void test_deny_to_smart_operation_in_first_repo_should_show_notification(String operation) {
    ...
    checkoutOrMerge(operation, "feature", [
            showSmartOperationDialog : { Project p, List<Change> cs, String op, boolean f -> BzrSmartOperationDialog.CANCEL_EXIT_CODE },
            notifyError: { String title, String message -> errorMessage = message }
    ] as BzrBranchUiHandler )
  ...
  void "Force checkout in case of local changes that would be overwritten by checkout"() {
    ...
    def uiHandler = [
      showSmartOperationDialog: { Project p, List<Change> cs, String op, boolean force ->
        BzrSmartOperationDialog.FORCE_EXIT_CODE;
      },
      notifySuccess: { String message -> successMessage = message },
      notifyError: { String title, String message -> errorMessage = message }
    ] as BzrBranchUiHandler
    BzrBranchWorker brancher = new BzrBranchWorker(myProject, myPlatformFacade, myBzr, uiHandler)
  ...
  public void "rollback of 'checkout branch as new branch' should delete branches"() {
    ...
    bzr("add feature.txt")
    bzr("commit -m feature_changes")
    bzr("checkout master")

    unmergedFiles(myCommunity)

    boolean rollbackProposed = false;
    BzrBranchWorker brancher = new BzrBranchWorker(myProject, myPlatformFacade, myBzr, [
  ...
  public void "delete branch that is fully merged should go without problems"() {
    myRepositories.each { cd it ; bzr("branch todelete") }
  ...
  public void "merge branch that is up-to-date"() {
    myRepositories.each { cd it ; bzr("branch master2") }
  ...
  public void "merge one simple and other up to date"() {
    ...
    [myUltimate, myContrib].each { cd it ; bzr("branch master2") }
  ...
  private String tip(BzrRepository repo) {
    cd repo
    bzr("rev-list -1 HEAD")
  }
  ...
  public void "deny rollback merge should leave as is"() {
    branchWithCommit(myRepositories, "feature")
    cd myUltimate
    String ultimateTipAfterMerge = bzr("rev-list -1 feature")
    unmergedFiles(myCommunity)
  ...
  def assertCurrentBranch(BzrRepository repository, String name) {
    def curBranch = bzr(repository, "branch").split("\n").find { it -> it.contains("*") }.replace('*', ' ').trim()
  ...
  def checkoutNewBranch(String name, def uiHandler) {
    BzrBranchWorker brancher = new BzrBranchWorker(myProject, myPlatformFacade, myBzr, uiHandler as BzrBranchUiHandler)
  ...
  def mergeBranch(String name, def uiHandler) {
    BzrBranchWorker brancher = new BzrBranchWorker(myProject, myPlatformFacade, myBzr, uiHandler as BzrBranchUiHandler)
    brancher.merge(name, BzrBrancher.DeleteOnMergeOption.PROPOSE, myRepositories)
  }
  ...
  private void prepareUnmergedBranch(BzrRepository unmergedRepo) {
    myRepositories.each {
      bzr(it, "branch todelete")
    }
    cd unmergedRepo
    bzr("checkout todelete")
    touch("afile.txt", "content")
    bzr("add afile.txt")
    bzr("commit -m unmerged_commit")
    bzr("checkout master")
  }
  ...
  def assertBranchDeleted(BzrRepository repo, String branch) {
    assertFalse("Branch $branch should have been deleted from $repo", bzr(repo, "branch").contains(branch))
  }

  def assertBranchExists(BzrRepository repo, String branch) {
    assertTrue("Branch $branch should exist in $repo", branchExists(repo, branch))
  }

  private void assertFile(BzrRepository repository, String path, String content) throws IOException {
    cd repository
  ...
  class AgreeToSmartOperationTestUiHandler implements BzrBranchUiHandler {
    ...
    int showSmartOperationDialog(@NotNull Project project, @NotNull List<Change> changes, @NotNull String operation, boolean force) {
      BzrSmartOperationDialog.SMART_EXIT_CODE
    }

    @Override
    boolean showBranchIsNotFullyMergedDialog(
      Project project,
      Map<BzrRepository, List<BzrCommit>> history,
    ...
    @Override
    void showUnmergedFilesNotification(@NotNull String operationName, @NotNull Collection<BzrRepository> repositories) {
      throw new UnsupportedOperationException()
    }
