 ...
 */
@Ignore
class BzrCrlfProblemsDetectorTest extends BzrLightTest {
  ...
  public void setUp() {
    ...
    cd myProjectRoot
    try {
      myOldGlobalAutoCrlfValue = bzr ("config --global core.autocrlf")
      bzr ("config --global --unset core.autocrlf")
    } catch (Exception e) {
      System.out.println("Couldn't operate with global config: " + e.getMessage())
    }
    myOldSystemAutoCrlfValue = bzr ("config --system core.autocrlf")
    bzr ("config --system --unset core.autocrlf")
  ...
  public void tearDown() {
    if (!StringUtil.isEmptyOrSpaces(myOldGlobalAutoCrlfValue)) {
      bzr ("config --global core.autocrlf " + myOldGlobalAutoCrlfValue);
    }
    if (!StringUtil.isEmptyOrSpaces(myOldSystemAutoCrlfValue)) {
      bzr ("config --global core.autocrlf " + myOldSystemAutoCrlfValue);
    }
    super.tearDown();
  }
...
"""
    ...
    assertTrue "Warning should be done, since one of the files has CRLFs and no related attributes",
           BzrCrlfProblemsDetector.detect(myProject, myPlatformFacade, myBzr,
  ...
  private BzrCrlfProblemsDetector detect(String relPath) {
    return detect(createVirtualFile(relPath))
  }

  private BzrCrlfProblemsDetector detect(VirtualFile file) {
    BzrCrlfProblemsDetector.detect(myProject, myPlatformFacade, myBzr, Collections.singleton(file))
  }
