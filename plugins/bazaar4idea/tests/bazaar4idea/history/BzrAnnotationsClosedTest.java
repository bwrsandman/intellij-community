 ...
 * Time: 1:00 PM
 */
public class BzrAnnotationsClosedTest extends BzrTest {
  ...
  private void imitateEvent(final VirtualFile root) {
    final BzrRepository repository = BzrUtil.getRepositoryManager(myProject).getRepositoryForRoot(root);
    repository.update();
  }
  ...
  private void imitInternalUpdate() {
    ...
    ex.setDirectoryMappings(Collections.singletonList(
      new VcsDirectoryMapping(FileUtil.toSystemIndependentName(myWorkingCopyDir.getPath()), BzrVcs.NAME)));
