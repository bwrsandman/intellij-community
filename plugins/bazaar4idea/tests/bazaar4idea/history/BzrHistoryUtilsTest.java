...
import static bazaar4idea.BzrUtil.getShortHash;
import static bazaar4idea.test.BzrTestRepository.createFile;
import static org.testng.Assert.*;

/**
 * Tests for low-level history methods in BzrHistoryUtils.
 ...
 */
public class BzrHistoryUtilsTest extends BzrTest {

  private VirtualFile afile;
  private FilePath bfilePath;
  private VirtualFile bfile;
  private List<BzrTestRevision> myRevisions;
  private List<BzrTestRevision> myRevisionsAfterRename;

  @BeforeMethod
  @Override
  public void setUp(Method testMethod) throws Exception {
    super.setUp(testMethod);
    myRevisions = new ArrayList<BzrTestRevision>(7);
    myRevisionsAfterRename = new ArrayList<BzrTestRevision>(4);

    // 1. create a file
    // 2. simple edit with a simple comit message
    // 3. move & rename
    // 4. make 4 edits with commit messages of different complexity
    // (note: after rename, because some BzrHistoryUtils methods don't follow renames).
    ...
    for (int i = revisions.length - 1, j = 0; i >= 0; i--, j++) {
      ...
      final BzrTestRevision revision = new BzrTestRevision(details[0], details[1], parents, commitMessages[i],
  ...
  void testCyclicRename() throws Exception {
    ...
    assertNotNull(vFile);
    List<VcsFileRevision> history = BzrHistoryUtils.history(myProject, new FilePathImpl(vFile));
  ...
  public void testGetCurrentRevision() throws Exception {
    BzrRevisionNumber revisionNumber = (BzrRevisionNumber) BzrHistoryUtils.getCurrentRevision(myProject, bfilePath, null);
  ...
  public void testGetLastRevisionForExistingFile() throws Exception {
    final ItemLatestState state = BzrHistoryUtils.getLastRevision(myProject, bfilePath);
  ...
  public void testHistory() throws Exception {
        List<VcsFileRevision> revisions = BzrHistoryUtils.history(myProject, bfilePath);
    assertEquals(revisions.size(), myRevisions.size());
    for (int i = 0; i < revisions.size(); i++) {
      assertEqualRevisions((BzrFileRevision) revisions.get(i), myRevisions.get(i));
    }
  }
  ...
  public void testAppendableHistory() throws Exception {
    ...
    BzrHistoryUtils.history(myProject, bfilePath, null, consumer, exceptionConsumer);
    assertEquals(revisions.size(), myRevisions.size());
    for (int i = 0; i < revisions.size(); i++) {
  ...
  public void testOnlyHashesHistory() throws Exception {
    final List<Pair<SHAHash,Date>> history = BzrHistoryUtils.onlyHashesHistory(myProject, bfilePath, myRepo.getVFRootDir());
    ...
    for (Iterator hit = history.iterator(), myIt = myRevisionsAfterRename.iterator(); hit.hasNext(); ) {
      ...
      BzrTestRevision revision = (BzrTestRevision)myIt.next();
  ...
  public void testHistoryWithLinks() throws Exception {
    /*List<BzrCommit> commits = BzrHistoryUtils.historyWithLinks(myProject, bfilePath, Collections.<String>emptySet(), null);
    ...
    for (Iterator hit = commits.iterator(), myIt = myRevisionsAfterRename.iterator(); hit.hasNext(); ) {
      BzrCommit commit = (BzrCommit)hit.next();
      BzrTestRevision revision = (BzrTestRevision)myIt.next();
  ...
  public void testHashesWithParents() throws Exception {
    ...
    BzrHistoryUtils.hashesWithParents(myProject, bfilePath, consumer, null, null);
    ...
    for (Iterator hit = hashesWithParents.iterator(), myIt = myRevisionsAfterRename.iterator(); hit.hasNext(); ) {
      ...
      BzrTestRevision rev = (BzrTestRevision)myIt.next();
  ...
  private static void assertEqualRevisions(BzrFileRevision actual, BzrTestRevision expected) throws IOException {
    assertEquals(((BzrRevisionNumber) actual.getRevisionNumber()).getRev(), expected.myHash);
    assertEquals(((BzrRevisionNumber) actual.getRevisionNumber()).getTimestamp(), expected.myDate);
  ...
  private static void assertCommitEqualToTestRevision(BzrHeavyCommit commit, BzrTestRevision expected) throws IOException {
  ...
  private static void assertCommitsEqualToTestRevisions(Collection<BzrHeavyCommit> actualCommits, Collection<BzrTestRevision> expectedRevisions) throws IOException {
    ...
    for (Iterator hit = actualCommits.iterator(), myIt = expectedRevisions.iterator(); hit.hasNext(); ) {
      BzrHeavyCommit commit = (BzrHeavyCommit)hit.next();
      BzrTestRevision revision = (BzrTestRevision)myIt.next();
      assertCommitEqualToTestRevision(commit, revision);
    }
  }
  ...
  private static class BzrTestRevision {
    final String myHash;
    final Date myDate;
