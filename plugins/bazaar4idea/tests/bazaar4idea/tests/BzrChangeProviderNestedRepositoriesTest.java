 ...
 */
@Deprecated
public class BzrChangeProviderNestedRepositoriesTest extends BzrChangeProviderTest {
  private File myChildRepoDir;
  private BzrTestRepository myChildRepo;
  ...
  public void setUp() throws Exception {
    ...
    myChildRepo = BzrTestRepository.init(myChildRepoDir);
    ...
    myChildFiles = BzrTestUtil.createFileStructure(myProject, myChildRepo, "in1.txt", "in2.txt", "dirr/inin1.txt", "dirr/inin2.txt");
    myChildRepo.addCommit();
    myChildRepo.refresh();

    myVcsManager = ProjectLevelVcsManager.getInstance(myProject);
    myVcsManager.setDirectoryMappings(Arrays.asList(new VcsDirectoryMapping(myRepo.getRootDir().getPath(), BzrVcs.getKey().getName()),
                                                    new VcsDirectoryMapping(myChildRepo.getRootDir().getPath(), BzrVcs.getKey().getName())));
