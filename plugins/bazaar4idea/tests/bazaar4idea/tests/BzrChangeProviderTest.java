...
/**
 * Tests BzrChangeProvider functionality. Scenario is the same for all tests:
 ...
 */
@Deprecated
public class BzrChangeProviderTest extends BzrTest {

  protected BzrChangeProvider myChangeProvider;
  ...
  protected void setUp(Method testMethod) throws Exception {
    ...
    myChangeProvider = (BzrChangeProvider) BzrVcs.getInstance(myProject).getChangeProvider();

    myFiles = BzrTestUtil.createFileStructure(myProject, myRepo, "a.txt", "b.txt", "dir/c.txt", "dir/subdir/d.txt");
    myRepo.addCommit();
    myRepo.refresh();
    ...
    myDirtyScope = new MockDirtyScope(myProject, BzrVcs.getInstance(myProject));
  }
