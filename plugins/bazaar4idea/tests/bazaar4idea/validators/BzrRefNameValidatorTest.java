...
/**
 * The test for {@link BzrRefNameValidator}.
 * Tests are based on the <a href="http://www.kernel.org/pub/software/scm/bzr/docs/bzr-check-ref-format.html">
 * specification of valid Bazaar references</a>
 *
 * @author Kirill Likhodedov
 */
public class BzrRefNameValidatorTest {
  ...
  private static void assertValid(String branchName) {
    assertTrue(BzrRefNameValidator.getInstance().checkInput(branchName), "Should be valid");
    assertTrue(BzrRefNameValidator.getInstance().canClose(branchName), "Should be valid");
  }

  private static void assertInvalid(String branchName) {
    assertFalse(BzrRefNameValidator.getInstance().checkInput(branchName), "Should be invalid");
    assertFalse(BzrRefNameValidator.getInstance().canClose(branchName), "Should be invalid");
  }
